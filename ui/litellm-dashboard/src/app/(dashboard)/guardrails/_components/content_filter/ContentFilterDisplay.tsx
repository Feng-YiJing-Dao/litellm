          ...
          <CardContent>
            <div className="mb-4 flex items-center justify-between">
              <p className="text-lg font-semibold">
                {t("content_filter.categories_title", { defaultValue: "Content Categories" })}
              </p>
              <Badge variant="secondary">
                {t("content_filter.categories_configured", {
                  count: categories.length,
                  defaultValue: `${categories.length} categories configured`,
                })}
              </Badge>
            </div>
            <CategoryTable
      ...
      {patterns.length > 0 && (
        <Card className="mt-6">
          <CardContent>
            <div className="mb-4 flex items-center justify-between">
              <p className="text-lg font-semibold">
                {t("content_filter.patterns_title", { defaultValue: "Pattern Detection" })}
              </p>
              <Badge variant="secondary">
                {t("content_filter.patterns_configured", {
                  count: patterns.length,
                  defaultValue: `${patterns.length} patterns configured`,
                })}
              </Badge>
            </div>
            <PatternTable
      ...
      {blockedWords.length > 0 && (
        <Card className="mt-6">
          <CardContent>
            <div className="mb-4 flex items-center justify-between">
              <p className="text-lg font-semibold">
                {t("content_filter.keywords_title", { defaultValue: "Blocked Keywords" })}
              </p>
              <Badge variant="secondary">
                {t("content_filter.keywords_configured", {
                  count: blockedWords.length,
                  defaultValue: `${blockedWords.length} keywords configured`,
                })}
              </Badge>
            </div>
            <KeywordTable
