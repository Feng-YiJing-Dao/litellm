  ...
  return (
    <div className="flex flex-col gap-6 min-w-0 w-full p-4">
      {conversationBlocks.map((block, blockIndex) => {
        ...
        const displayModel = assistantMessage?.model || t("playground:compare.assistant", "Assistant");
        return (
          <div key={blockIndex} className="space-y-4">
            ...
            {assistantMessage ? (
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span>{t("playground:compare.generating_response", "Generating response...")}</span>
              </div>
            ) : (
              <div className="text-sm text-muted-foreground">
                {t("playground:compare.waiting_response", "Waiting for a response...")}
              </div>
            )}
          </div>
        );
