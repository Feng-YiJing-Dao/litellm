        ...
        <div className="border-b px-4 py-2">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              >
                <SelectTrigger className="w-48" aria-label={t("playground:virtual_key_source", { defaultValue: "Virtual Key Source" })}>
                  <SelectValue>{apiKeySource === "custom" ? t("playground:virtual_key", { defaultValue: "Virtual Key" }) : t("playground:current_ui_session", { defaultValue: "Current UI Session" })}</SelectValue>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="session" disabled={!canUseSessionKey}>
                    {t("playground:current_ui_session", { defaultValue: "Current UI Session" })}
                  </SelectItem>
                  <SelectItem value="custom">{t("playground:virtual_key", { defaultValue: "Virtual Key" })}</SelectItem>
                </SelectContent>
              </Select>
              {apiKeySource === "custom" && (
                <Input
                  ...
                  placeholder={t("playground:enter_key", { defaultValue: "Enter Virtual Key" })}
                  className="w-56"
                />
              )}
            </div>
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-muted-foreground">
                {t("playground:endpoint_type", { defaultValue: "Endpoint" })}
              </span>
              <Select value={selectedEndpoint} onValueChange={(value) => setSelectedEndpoint(value as EndpointIdType)}>
                <SelectTrigger className="w-56" aria-label={t("playground:endpoint_type", { defaultValue: "Endpoint" })}>
            ...
            <div className="flex items-center gap-3">
              <Button variant="outline" onClick={clearAllChats} disabled={!hasMessages}>
                <Eraser />
                {t("playground:clear_all_chats", { defaultValue: "Clear All Chats" })}
              </Button>
              <Tooltip>
                <TooltipTrigger render={<span className="inline-flex" />}>
                  <Button variant="outline" onClick={addComparison} disabled={comparisons.length >= maxComparisons}>
                    <Plus />
                    {t("playground:add_comparison", { defaultValue: "Add Comparison" })}
                  </Button>
                </TooltipTrigger>
                <TooltipContent>
                  {comparisons.length >= maxComparisons
                    ? t("playground:compare_limit", { defaultValue: "Compare up to 3 models at a time" })
                    : t("playground:add_another_comparison", { defaultValue: "Add another comparison" })}
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between gap-4 mb-3 min-h-8">
                {hasAttachment ? (
                  <span className="text-sm text-muted-foreground">
                    {t("playground:attachment_ready", { defaultValue: "Attachment ready to send" })}
                  </span>
                ...
                <div className="mb-3">
                  <div className="flex items-center gap-3 p-3 bg-muted rounded-lg border border-border">
                    <div className="relative inline-block">
                      {isUploadedFilePdf ? (
                        ...
                        <img
                          ...
                          alt={t("playground:upload_preview", { defaultValue: "Upload preview" })}
                    ...
                    <button
                      ...
                      aria-label={t("playground:remove_attachment", { defaultValue: "Remove attachment" })}
                    >
