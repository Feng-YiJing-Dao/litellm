  ...
  const handleOk = form.handleSubmit(
    (values) => {
      ...
      editMutation.mutate(
        ...
        {
          onSuccess: () => {
            toast.success(t("accessGroups:edit_modal.updated_success", { defaultValue: "Access group updated successfully" }));
            onSuccess?.();
            onCancel();
          },
        },
    ...
    <form onSubmit={(event) => event.preventDefault()}>
      ...
      <div className="mt-6 flex justify-end gap-2">
        ...
        <Button type="button" onClick={() => void handleOk()} disabled={editMutation.isPending}>
          {t("accessGroups:edit_modal.save", { defaultValue: "Save Changes" })}
        </Button>
      </div>
    </form>
  );
  ...
  return (
    <Dialog open={visible} onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[700px]">
        <DialogHeader>
          <DialogTitle>{t("accessGroups:edit_modal.title", { defaultValue: "Edit Access Group" })}</DialogTitle>
        </DialogHeader>
        <AccessGroupEditForm
