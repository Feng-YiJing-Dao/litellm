      ...
      <div className="grid md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <div className="flex flex-col items-center justify-center gap-2 p-6">
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            {t("auto_router_benchmarks.total_estimated_savings", { defaultValue: "Total estimated savings" })}
          </p>
        ...
        <div className="flex flex-col justify-center border-t p-6 md:border-t-0 md:border-l">
          <SpendRow
            label={t("auto_router_benchmarks.actual_spend", { defaultValue: "Actual auto-router spend" })}
            value={usd(stats.spend)}
          />
          <div className="mb-3 border-l-2 pl-4">
            <SpendRow
              subdued
              label={t("auto_router_benchmarks.llm_spend", { defaultValue: "LLM spend" })}
              value={
                stats.classifier_cost == null
                  ? t("auto_router_benchmarks.unavailable_label", { defaultValue: "Unavailable" })
                  : usd(stats.spend - stats.classifier_cost)
              }
            />
            <SpendRow
              subdued
              label={t("auto_router_benchmarks.classification_cost", { defaultValue: "Classification cost" })}
              value={
                stats.classifier_cost == null
                  ? t("auto_router_benchmarks.unavailable_label", { defaultValue: "Unavailable" })
          ...
          {stats.classifier_cost == null && (
            <p className="mb-3 text-xs text-muted-foreground">
              {t("auto_router_benchmarks.breakdown_unavailable", {
          ...
          <Separator />
          <SpendRow
            label={t("auto_router_benchmarks.baseline_spend", {
  ...
  return (
    <div className="flex flex-col gap-1">
      <div
        ...
        role="img"
        aria-label={t("auto_router_benchmarks.share_of_turns_by_bucket", {
      ...
      <TableHeader>
        <TableRow className="hover:bg-transparent">
          <TableHead className="text-[11px] uppercase tracking-wide">
            {t("auto_router_benchmarks.bucket_col", { defaultValue: "Bucket" })}
          </TableHead>
          <TableHead className="text-right text-[11px] uppercase tracking-wide">
            {t("auto_router_benchmarks.turns_col", { defaultValue: "Turns" })}
          </TableHead>
          <TableHead className="w-1/2" />
          <TableHead className="text-right text-[11px] uppercase tracking-wide">
            {t("auto_router_benchmarks.hit_rate_col", { defaultValue: "Hit rate" })}
          </TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        ...
        <div className="flex flex-col border-b p-6 lg:border-b-0 lg:border-r">
          <div className="flex flex-1 flex-col justify-center gap-3">
            <p className="text-sm text-muted-foreground">
              {t("auto_router_benchmarks.cache_hit_rate", { defaultValue: "Cache hit rate" })}
            </p>
              ...
              <Tooltip>
                ...
                >
                  <span className="text-sm text-muted-foreground underline decoration-dotted underline-offset-2">
                    {t("auto_router_benchmarks.expired_miss", { defaultValue: "Expired-miss" })}
                  </span>
                  <span className="font-medium tabular-nums text-foreground">{pctLabel(expiredMissPct)}</span>
                </TooltipTrigger>
                <TooltipContent className="max-w-64">
                  {t("auto_router_benchmarks.expired_miss_tooltip", {
                    defaultValue:
        ...
        <div className="flex flex-col gap-3 p-6">
          <div className="flex items-baseline justify-between">
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
              {t("auto_router_benchmarks.share_of_turns", { defaultValue: "Share of turns" })}
            </p>
            <p className="text-xs text-muted-foreground">
              <span className="text-lg font-semibold tabular-nums text-foreground">{total.toLocaleString()}</span>{" "}
              {t("auto_router_benchmarks.turns_measured", {
                count: total,
                defaultValue: `${total.toLocaleString()} turns measured`,
              }).replace(`${total.toLocaleString()} `, "")}
            </p>
          </div>
          ...
          {cache.unordered_turns > 0 && (
            <p className="text-xs text-muted-foreground">
              {t("auto_router_benchmarks.unordered_turns_note", {
                count: cache.unordered_turns,
                defaultValue: `${cache.unordered_turns} turns arrived out of order across pods and are not bucketed`,
              })}
            </p>
          )}
        </div>
...
const BenchmarksBody: React.FC<BenchmarksBodyProps> = ({ isPending, error, data, selectedKey, autoRouters }) => {
  const { t } = useTranslation("costs");
  if (isPending) return <Message>{t("auto_router_benchmarks.loading", { defaultValue: "Loading auto-router usage..." })}</Message>;
  if (error instanceof ApiError && error.status === 403) {
    return (
      <Message>
        {t("auto_router_benchmarks.admin_only", { defaultValue: "Auto-router usage is visible to proxy admin roles only" })}
      </Message>
    );
  }
  if (error || !data) {
    return (
      <Message>
        {t("auto_router_benchmarks.unavailable", { defaultValue: "Auto-router usage is unavailable right now" })}
      </Message>
    );
  }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Metric
          label={t("auto_router_benchmarks.avg_saved_per_session", { defaultValue: "Avg saved per session" })}
          value={usd(stats.saved_per_session)}
          hint={t("auto_router_benchmarks.sessions_hint", {
            count: stats.sessions,
            defaultValue: `· ${stats.sessions} sessions`,
          })}
        />
        <Metric
          label={t("auto_router_benchmarks.avg_turns_per_session", { defaultValue: "Avg turns per session" })}
          value={stats.avg_turns_per_session.toFixed(1)}
        />
        <Metric
          label={t("auto_router_benchmarks.avg_session_length", { defaultValue: "Avg session length" })}
          value={durationLabel(stats.avg_session_seconds)}
        />
        <Metric
          label={t("auto_router_benchmarks.avg_tokens_per_session", { defaultValue: "Avg tokens per session" })}
          value={formatNumberWithCommas(stats.avg_tokens_per_session, 1, true)}
        />
      </div>

      <p className="text-xs text-muted-foreground">
        {t("auto_router_benchmarks.methodology_note", {
          defaultValue:
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-baseline gap-2">
          <h3 className="text-lg font-semibold text-foreground">
            {t("auto_router_benchmarks.prompt_caching_title", { defaultValue: "Auto-router prompt caching" })}
          </h3>
          <p className="text-xs text-muted-foreground">
            {t("auto_router_benchmarks.prompt_caching_subtitle", {
...
export const AutoRouterUsageView: React.FC<AutoRouterBenchmarksTabProps> = ({ accessToken, activity, apiKey }) => {
  ...
  const selectedLabel = data
    ? viewFor(data, selectedKey).label
    : t("auto_router_benchmarks.all_routers", { defaultValue: "All auto-routers" });
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-foreground">
            {t("auto_router_benchmarks.title", { defaultValue: "Auto-router usage" })}
          </h2>
    ...
    <Tabs defaultValue="usage" onValueChange={handleTabChange} className="w-full gap-4">
      <TabsList>
        <TabsTrigger value="usage" className="px-3">
          {t("auto_router_benchmarks.tab_usage", { defaultValue: "Usage" })}
        </TabsTrigger>
        <TabsTrigger value="shadow-evals" className="px-3">
          {t("auto_router_benchmarks.tab_shadow_evals", { defaultValue: "Shadow Evals" })}
        </TabsTrigger>
      </TabsList>
