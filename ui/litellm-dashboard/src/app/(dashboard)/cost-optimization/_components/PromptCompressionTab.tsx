...
const PromptCompressionTab: React.FC<PromptCompressionTabProps> = ({ accessToken }) => {
  ...
  const loadGuardrails = useCallback(() => {
    ...
    getGuardrailsList(accessToken)
      ...
      .catch((error) => {
        ...
        toast.fromError(t("costs:compression.load_failed", "Failed to load compression guardrails"));
      })
  ...
  const handleAdd = async (values: CompressionFormValues) => {
    ...
    try {
      ...
      toast.success(t("costs:compression.created_success", "Compression guardrail created"));
      form.reset(EMPTY_VALUES);
      await loadGuardrails();
    } catch (error) {
      console.error("Failed to create compression guardrail:", error);
      toast.fromError(t("costs:compression.create_failed", "Failed to create compression guardrail"));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="w-full space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>{t("costs:compression.title", "Headroom prompt compression")}</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="mb-4 text-sm text-muted-foreground">
            {t(
              "costs:compression.description",
              "Headroom is a native LiteLLM guardrail that compresses your prompts before they reach the model, so you pay for fewer input tokens. The tokens it removes are priced and shown on the Usage tab as compression savings.",
            )}{" "}
            <a
            ...
            >
              {t("costs:compression.setup_docs", "Headroom setup docs")}
            </a>
          </p>
            ...
            <ul className="divide-y divide-border">
              {guardrails.map((guardrail) => (
                <li key={guardrail.guardrail_id} className="flex items-center justify-between py-3">
                  ...
                  >
                    {guardrail.litellm_params?.default_on
                      ? t("costs:compression.always_on", "Always on")
                      : t("costs:compression.opt_in", "Opt-in")}
                  </span>
                </li>
              ))}
            </ul>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("costs:compression.add_title", "Add Headroom compression guardrail")}</CardTitle>
        </CardHeader>
        <CardContent>
          <TooltipProvider>
            <form onSubmit={form.handleSubmit(handleAdd)} noValidate>
              <FieldGroup>
                <FormField control={form.control} name="name" label={t("common:name", "Name")}>
                ...
                <FormField
                  ...
                  name="apiBase"
                  label={labelWithHint(
                    t("costs:compression.api_base_label", "Headroom API base"),
                    t(
                      "costs:compression.api_base_hint",
                      "Base URL of your Headroom compression service (LiteLLM calls its /v1/compress endpoint)",
                    ),
                  )}
                  description={t("costs:compression.api_base_desc", "The URL where your Headroom compression service is hosted")}
                >
                ...
                <FormField
                  control={form.control}
                  name="defaultOn"
                  label={t("costs:compression.default_on_label", "Apply to all requests")}
                >
              ...
              <div className="mt-6 mb-4 rounded-lg border border-warning/20 bg-warning/10 p-3">
                <p className="text-sm text-warning">
                  {t(
                    "costs:compression.enterprise_notice",
                    "Applying compression to all requests is available to all users. Enabling it selectively per key or team is a LiteLLM Enterprise feature. Get a trial key",
                  )}{" "}
                  <a
              ...
              <div className="flex justify-end">
                <Button type="submit" disabled={isSaving}>
                  ...
                  {t("costs:compression.add_guardrail", "Add guardrail")}
                </Button>
              </div>
            </form>
          </TooltipProvider>
