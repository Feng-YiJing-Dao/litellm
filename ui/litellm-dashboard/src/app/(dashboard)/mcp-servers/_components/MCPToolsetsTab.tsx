        ...
        <span className="text-sm font-medium text-foreground flex items-center gap-2">
          ...
          {selectedSet.size > 0 && (
            <span className="ml-1 text-xs text-purple-600 font-semibold dark:text-purple-400">
              {t("toolsets.selected_count", {
                count: selectedSet.size,
                defaultValue: `${selectedSet.size} selected`,
              })}
            </span>
          )}
        </span>
        ...
        <div className="p-2">
          {loading ? (
            ...
            <p className="text-xs text-muted-foreground px-2 py-2">
              {t("toolsets.no_tools_found", { defaultValue: "No tools found for this server." })}
            </p>
          ) : (
    ...
    <Dialog open={open} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[960px]">
        <DialogHeader>
          <DialogTitle>
            {initialToolset
              ? t("toolsets.edit_toolset", { defaultValue: "Edit Toolset" })
              : t("toolsets.new_toolset", { defaultValue: "New Toolset" })}
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={(event) => event.preventDefault()} className="mt-2">
          <FieldGroup className="mb-4 flex-row gap-4">
            <FormField
              ...
              name="toolset_name"
              label={t("toolsets.toolset_name", { defaultValue: "Toolset Name" })}
              className="flex-1"
            >
              {(field) => (
                <Input
                  {...field}
                  placeholder={t("toolsets.toolset_name_placeholder", { defaultValue: "e.g. github-linear-tools" })}
                />
              )}
            </FormField>
            <FormField
              control={form.control}
              name="description"
              label={t("toolsets.description", { defaultValue: "Description" })}
              className="flex-1"
            >
              {(field) => (
                <Input
                  {...field}
                  placeholder={t("toolsets.description_placeholder", { defaultValue: "Optional description" })}
                />
              )}
            </FormField>
          </FieldGroup>
        ...
        <div className="flex gap-4 mt-2" style={{ minHeight: 360 }}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-semibold text-foreground">
                {t("toolsets.available_tools", { defaultValue: "Available Tools" })}
              </p>
            </div>
            <InputGroup className="mb-2">
              <InputGroupInput
                placeholder={t("toolsets.search_servers", { defaultValue: "Search MCP servers..." })}
                value={serverSearch}
              ...
              {serverSearch && (
                <InputGroupAddon align="inline-end">
                  <InputGroupButton
                    size="icon-xs"
                    aria-label={t("toolsets.clear_search", { defaultValue: "Clear search" })}
            ...
            <div className="space-y-2 overflow-y-auto" style={{ maxHeight: 300 }}>
              {filteredServers.length === 0 ? (
                <p className="text-muted-foreground text-sm">
                  {mcpServers.length === 0
                    ? t("toolsets.no_servers_configured", { defaultValue: "No MCP servers configured" })
                    : t("toolsets.no_servers_match", { defaultValue: "No servers match your search" })}
                </p>
              ) : (
          ...
          <div className="w-72 shrink-0">
            <p className="text-sm font-semibold text-foreground mb-2 block">
              {t("toolsets.your_toolset", { defaultValue: "Your Toolset" })}{" "}
              <span className="text-xs font-normal text-muted-foreground">
                {t("toolsets.tools_count", {
                  count: selectedTools.length,
                  defaultValue: `(${selectedTools.length} tools)`,
                })}
              </span>
            </p>
            <div className="space-y-1 overflow-y-auto" style={{ maxHeight: 340 }}>
              {selectedTools.length === 0 ? (
                <p className="text-muted-foreground text-sm">
                  {t("toolsets.no_tools_added", { defaultValue: "No tools added yet" })}
                </p>
              ) : (
        ...
        <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-border">
          <Button variant="outline" onClick={onClose}>
            {t("toolsets.cancel", { defaultValue: "Cancel" })}
          </Button>
          <Button onClick={() => void form.handleSubmit(handleSubmit)()} disabled={saving} aria-busy={saving}>
            ...
            {initialToolset
              ? t("toolsets.save_changes", { defaultValue: "Save Changes" })
              : t("toolsets.create_toolset", { defaultValue: "Create Toolset" })}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">
        {t("toolsets.empty_title", { defaultValue: "No toolsets yet" })}
      </div>
      <div className="text-sm text-muted-foreground">
        {t("toolsets.empty_desc", {
  ...
  return (
    <div className="mb-6 rounded-lg border border-border bg-muted px-5 py-4">
      <p className="text-sm font-medium text-foreground mb-1">
        {t("toolsets.how_toolsets_work", { defaultValue: "How toolsets work" })}
      </p>
      <p className="text-sm text-muted-foreground mb-3">
        {t("toolsets.how_toolsets_work_desc", {
          defaultValue:
      ...
      <div className="text-xs text-muted-foreground mb-1">
        {t("toolsets.client_config_label", { defaultValue: "Claude Code / Cursor config" })}
      </div>
      <div className="relative">
        ...
        >
          {copied ? "✓" : t("toolsets.copy", { defaultValue: "copy" })}
        </button>
      </div>
    </div>
  );
...
export function MCPToolsetsTab({ accessToken, userRole }: MCPToolsetsTabProps) {
  ...
  const handleCreate = async (name: string, description: string | undefined, tools: MCPToolsetTool[]) => {
    ...
    toast.success(t("toolsets.toolset_created", { defaultValue: "Toolset created" }));
  ...
  const handleUpdate = async (name: string, description: string | undefined, tools: MCPToolsetTool[]) => {
    ...
    toast.success(t("toolsets.toolset_updated", { defaultValue: "Toolset updated" }));
    queryClient.invalidateQueries({ queryKey: ["mcpToolsets"] });
    setEditToolset(null);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(t("toolsets.toolset_deleted", { defaultValue: "Toolset deleted" }));
      queryClient.invalidateQueries({ queryKey: ["mcpToolsets"] });
      setDeleteId(null);
    } finally {
      setDeleting(false);
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-medium text-foreground">
            {t("toolsets.title", { defaultValue: "MCP Toolsets" })}
          </h3>
          <p className="text-muted-foreground text-sm">
            {t("toolsets.subtitle", {
              defaultValue:
      ...
      <DataTable
        ...
        isLoading={isLoading}
        loadingMessage={t("toolsets.loading_toolsets", { defaultValue: "Loading toolsets…" })}
      ...
      <Dialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("toolsets.delete_toolset", { defaultValue: "Delete Toolset" })}</DialogTitle>
          </DialogHeader>
          <p>
            {t("toolsets.delete_confirm", {
              defaultValue:
          ...
          <DialogFooter>
            ...
            <Button onClick={handleDelete} variant="destructive" disabled={deleting} aria-busy={deleting}>
              {t("toolsets.delete", { defaultValue: "Delete" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
