    ...
    <MountedFormField
      label={
        <span className="text-sm font-medium text-foreground flex items-center">
          {t("mcp:stdio.label", "Stdio Configuration (JSON)")}
          <SimpleTooltip content={t("mcp:stdio.tooltip", "Paste your stdio MCP server configuration in JSON format. You can use the full mcpServers structure from config.yaml or just the inner server configuration.")}>
      ...
      rules={{
        validate: {
          ...(required ? { required: requiredRule(t("mcp:stdio.required", "Please enter stdio configuration")) } : {}),
          json: parsesAsJson(t("mcp:stdio.invalid_json", "Please enter valid JSON")),
        },
      }}
    >
      {(control) => (
