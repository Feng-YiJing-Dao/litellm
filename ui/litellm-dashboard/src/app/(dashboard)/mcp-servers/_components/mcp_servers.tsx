...
const MCPServers: React.FC<MCPServerProps> = ({ accessToken, userRole, userID }) => {
  ...
  const sortOptions = useMemo(() => [
    { value: "created_desc" as SortKey, label: t("mcp:sort.created_desc", { defaultValue: "Recently created" }) },
    { value: "updated_desc" as SortKey, label: t("mcp:sort.updated_desc", { defaultValue: "Recently updated" }) },
    { value: "name_asc" as SortKey, label: t("mcp:sort.name_asc", { defaultValue: "Name (A→Z)" }) },
    { value: "health" as SortKey, label: t("mcp:sort.health", { defaultValue: "Health (unhealthy first)" }) },
  ], [t]);
    ...
    <TooltipProvider>
      <div className="h-full w-full p-6">
        <AlertDialog open={isDeleteModalOpen} onOpenChange={(open) => !open && cancelDelete()}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>{t("mcp:delete_modal.title", { defaultValue: "Delete MCP Server?" })}</AlertDialogTitle>
            </AlertDialogHeader>
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                {t("mcp:delete_modal.desc", {
                  defaultValue:
              ...
              {serverToDelete && (
                <dl className="mt-3 space-y-1 rounded-lg border border-border bg-muted p-4">
                  {serverToDelete.server_name && (
                    <div className="flex gap-2">
                      <dt className="text-sm text-muted-foreground">
                        {t("mcp:delete_modal.name", { defaultValue: "Name" })}
                      </dt>
                  ...
                  <div className="flex gap-2">
                    <dt className="text-sm text-muted-foreground">
                      {t("mcp:delete_modal.id", { defaultValue: "ID" })}
                    </dt>
                  ...
                  {serverToDelete.url && (
                    <div className="flex gap-2">
                      <dt className="text-sm text-muted-foreground">
                        {t("mcp:delete_modal.url", { defaultValue: "URL" })}
                      </dt>
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-semibold">{t("mcp:title", { defaultValue: "MCP Servers" })}</h1>
              {filteredServers.length > 0 && <Badge variant="secondary">{filteredServers.length}</Badge>}
            </div>
            <p className="mt-1 text-sm text-muted-foreground">{t("mcp:subtitle", { defaultValue: "Configure and manage your MCP servers" })}</p>
          </div>
          <div className="flex items-center gap-2">
            {isAdminRole(userRole) && (
              <>
                <Button className="shrink-0" variant="secondary" onClick={() => setImportVisible(true)}>
                  {t("mcp:import_json", { defaultValue: "Import from JSON" })}
                </Button>
                <Button className="shrink-0" onClick={() => setDiscoveryVisible(true)}>
                  {t("mcp:add_new_server", { defaultValue: "+ Add New MCP Server" })}
                </Button>
              </>
            )}
            {!isAdminRole(userRole) && (
              <Button
                ...
                variant="secondary"
              >
                {t("mcp:submit_server", { defaultValue: "+ Submit MCP Server" })}
              </Button>
            )}
          </div>
        </div>
              ...
              <div className="w-full h-full">
                <div className="w-full">
                  <div className="flex flex-col space-y-4">
                    <div className="flex items-center gap-6 rounded-lg border border-border bg-card px-4 py-3">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium whitespace-nowrap text-muted-foreground">{t("mcp:team", { defaultValue: "Team" })}</p>
                        <Select
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="all">
                              {isInternalUser ? t("mcp:all_available_servers", { defaultValue: "All Available Servers" }) : t("mcp:tabs.servers", { defaultValue: "All Servers" })}
                            </SelectItem>
                            <SelectItem value="personal">{t("mcp:personal", { defaultValue: "Personal" })}</SelectItem>
                      ...
                      <div className="flex items-center gap-2">
                        <p className="flex items-center text-sm font-medium whitespace-nowrap text-muted-foreground">
                          {t("mcp:access_group", { defaultValue: "Access Group" })}
                          <Tooltip>
                            <TooltipTrigger
                              render={
                                <CircleHelp
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="all">{t("mcp:all_access_groups", { defaultValue: "All Access Groups" })}</SelectItem>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  <InputGroup className="max-w-80">
                    ...
                    <InputGroupInput
                      placeholder={t("mcp:search_placeholder", { defaultValue: "Search by name, alias, URL, or ID" })}
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                    />
                  </InputGroup>
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium whitespace-nowrap text-muted-foreground">{t("mcp:sort.label", { defaultValue: "Sort" })}</p>
                    <Select
                      items={sortOptions}
                      value={sortKey}
                  ...
                  <div className="ml-auto text-xs text-muted-foreground">
                    {t("mcp:servers_count", { displayed: displayedServers.length, total: filteredServers.length, defaultValue: `${displayedServers.length} of ${filteredServers.length} servers` })}
                  </div>
                </div>
                <div className="mt-4 w-full">
                  {isLoadingServers ? (
                    <div className="flex items-center justify-center gap-3 rounded-lg border border-dashed border-border bg-card p-12">
                      ...
                      <p className="text-sm text-muted-foreground">{t("mcp:loading_servers", { defaultValue: "Loading MCP servers..." })}</p>
                    </div>
                  ) : displayedServers.length === 0 ? (
                    <div className="rounded-lg border border-dashed border-border bg-card p-12 text-center">
                      <p className="text-sm text-muted-foreground">
                        {filteredServers.length === 0
                          ? t("mcp:no_servers_configured", { defaultValue: "No MCP servers configured. Click '+ Add New MCP Server' to get started." })
                          : t("mcp:no_servers_match", { defaultValue: "No servers match the current filters or search." })}
                      </p>
                    </div>
                  ) : (
                    <div
