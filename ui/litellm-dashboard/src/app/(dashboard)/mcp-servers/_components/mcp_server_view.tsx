    ...
    <div className="max-w-full p-4">
      <div className="mb-6">
        <Button variant="ghost" className="mb-4" onClick={onBack}>
          <ArrowLeft />
          {t("mcp:view_server.back_to_servers", { defaultValue: "Back to All Servers" })}
        </Button>
        <div className="flex items-center gap-2">
          <h1 className="text-2xl font-semibold">
            {mcpServer.server_name || mcpServer.alias || t("mcp:view_server.unnamed_server", { defaultValue: "Unnamed Server" })}
          </h1>
          <Button
            variant="ghost"
            size="icon-sm"
            aria-label={t("mcp:view_server.copy_server_name", { defaultValue: "Copy server name" })}
        ...
        <div className="mt-1 flex items-center gap-1.5">
          ...
          <Button
            variant="ghost"
            size="icon-sm"
            aria-label={t("mcp:view_server.copy_server_id", { defaultValue: "Copy server id" })}
      ...
      <Tabs value={String(selectedTabIndex)} onValueChange={(v: unknown) => setSelectedTabIndex(Number(v))}>
        <TabsList variant="line" className="mb-4 h-auto w-full justify-start rounded-none border-b p-0">
          <TabsTrigger value="0" className="flex-none rounded-none px-4 py-2">
            {t("mcp:view_server.overview", { defaultValue: "Overview" })}
          </TabsTrigger>
          <TabsTrigger value="1" className="flex-none rounded-none px-4 py-2">
            {t("mcp:view_server.mcp_tools", { defaultValue: "MCP Tools" })}
          </TabsTrigger>
          {isProxyAdmin && (
            <TabsTrigger value="2" className="flex-none rounded-none px-4 py-2">
              {t("mcp:view_server.settings", { defaultValue: "Settings" })}
            </TabsTrigger>
          )}
        </TabsList>
        ...
        <TabsContent value="0" keepMounted>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <Card className="p-4">
              <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">
                {t("mcp:view_server.transport", { defaultValue: "Transport" })}
              </p>
              <div className="mt-3">
            ...
            <Card className="p-4">
              <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">
                {t("mcp:view_server.authentication", { defaultValue: "Authentication" })}
              </p>
            ...
            <Card className="p-4">
              <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">
                {t("mcp:view_server.host_url", { defaultValue: "Host URL" })}
              </p>
              <div className="mt-3 flex items-center gap-2">
                ...
                {hasToken && isProxyAdmin && (
                  <Button
                    ...
                    aria-label={
                      showFullUrl
                        ? t("mcp:view_server.hide_full_url", { defaultValue: "Hide full URL" })
                        : t("mcp:view_server.show_full_url", { defaultValue: "Show full URL" })
                    }
          ...
          <Card className="mt-4 p-4">
            <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">
              {t("mcp:view_server.cost_config", { defaultValue: "Cost Configuration" })}
            </p>
            <div className="mt-3">
        ...
        <TabsContent value="2" keepMounted>
          <Card className="p-6">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-medium">{t("mcp:view_server.settings_title", { defaultValue: "MCP Server Settings" })}</h2>
              {editing ? null : (
                <Button variant="outline" onClick={() => setEditing(true)}>
                  {t("mcp:view_server.edit_settings", { defaultValue: "Edit Settings" })}
                </Button>
              )}
            </div>
            {editing ? (
              ...
              <div className="divide-y divide-border">
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.server_name", { defaultValue: "Server Name" })}</p>
                  <div className="col-span-2 text-sm">
                    {mcpServer.server_name || <span className="text-muted-foreground">—</span>}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.alias", { defaultValue: "Alias" })}</p>
                  <div className="col-span-2 font-mono text-sm">
                    {mcpServer.alias || <span className="text-muted-foreground">—</span>}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.description", { defaultValue: "Description" })}</p>
                  <div className="col-span-2 text-sm">
                    {mcpServer.description || <span className="text-muted-foreground">—</span>}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.url", { defaultValue: "URL" })}</p>
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.transport", { defaultValue: "Transport" })}</p>
                  <div className="col-span-2">
                    {getTransportBadge(handleTransport(mcpServer.transport, mcpServer.spec_path))}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.authentication", { defaultValue: "Authentication" })}</p>
                  <div className="col-span-2">{getAuthBadge(handleAuth(mcpServer.auth_type))}</div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.extra_headers", { defaultValue: "Extra Headers" })}</p>
                  <div className="col-span-2 text-sm">
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.allow_all_keys", { defaultValue: "Allow All Keys" })}</p>
                  <div className="col-span-2">
                    {mcpServer.allow_all_keys ? (
                      <Badge variant="outline">
                        <span className="h-1.5 w-1.5 rounded-full bg-success" />
                        {t("mcp:view_server.enabled", { defaultValue: "Enabled" })}
                      </Badge>
                    ) : (
                      <Badge variant="outline">{t("mcp:view_server.disabled", { defaultValue: "Disabled" })}</Badge>
                    )}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.network_access", { defaultValue: "Network Access" })}</p>
                  <div className="col-span-2">
                    {mcpServer.available_on_public_internet ? (
                      <Badge variant="outline">
                        <span className="h-1.5 w-1.5 rounded-full bg-success" />
                        {t("mcp:view_server.public", { defaultValue: "Public" })}
                      </Badge>
                    ) : (
                      <Badge variant="outline">
                        <span className="h-1.5 w-1.5 rounded-full bg-warning" />
                        {t("mcp:view_server.internal_only", { defaultValue: "Internal only" })}
                      </Badge>
                    )}
                  </div>
                </div>
                {handleAuth(mcpServer.auth_type) === "oauth2" && (
                  <div className="grid grid-cols-3 gap-4 py-3">
                    <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.delegate_auth", { defaultValue: "Delegate Auth to Upstream" })}</p>
                    <div className="col-span-2">
                      {mcpServer.delegate_auth_to_upstream ? (
                        <Badge variant="outline">
                          <span className="h-1.5 w-1.5 rounded-full bg-success" />
                          {t("mcp:view_server.pkce_passthrough", { defaultValue: "Enabled (PKCE passthrough)" })}
                        </Badge>
                      ) : (
                        <Badge variant="outline">{t("mcp:view_server.disabled", { defaultValue: "Disabled" })}</Badge>
                      )}
                ...
                {handleAuth(mcpServer.auth_type) !== "oauth2" &&
                  ...
                  mcpServer.extra_headers.some((h) => typeof h === "string" && h.toLowerCase() === "authorization") && (
                    <div className="grid grid-cols-3 gap-4 py-3">
                      <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.oauth_passthrough", { defaultValue: "OAuth Pass-through" })}</p>
                      <div className="col-span-2">
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.access_groups", { defaultValue: "Access Groups" })}</p>
                  <div className="col-span-2">
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.allowed_tools", { defaultValue: "Allowed Tools" })}</p>
                  <div className="col-span-2">
                    {mcpServer.allowed_tools && mcpServer.allowed_tools.length > 0 ? (
                      ...
                      <Badge variant="outline">{t("mcp:view_server.all_tools_enabled", { defaultValue: "All tools enabled" })}</Badge>
                    )}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcp:view_server.cost", { defaultValue: "Cost" })}</p>
                  <div className="col-span-2">
                    <MCPServerCostDisplay costConfig={mcpServer.mcp_info?.mcp_server_cost_info} />
                  </div>
                </div>
