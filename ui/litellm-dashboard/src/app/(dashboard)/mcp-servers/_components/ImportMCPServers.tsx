  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (response.imported.length > 0) {
        toast.success(
          t("import_connectors.imported_toast", {
            count: response.imported.length,
            defaultValue: `Imported ${response.imported.length} MCP server${response.imported.length === 1 ? "" : "s"}`,
          }),
        );
        onImported();
      }
      ...
      setParseError(
        t("import_connectors.failed_request", {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && handleClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("import_connectors.title", { defaultValue: "Import MCP Connectors" })}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            {t("import_connectors.desc", {
              defaultValue:
          ...
          {result && (
            <div className="space-y-2 text-sm">
              {result.imported.length > 0 && (
                <div>
                  <span className="font-semibold">
                    {t("import_connectors.imported", { defaultValue: "Imported:" })}
                  </span>{" "}
              ...
              {result.skipped.length > 0 && (
                <div>
                  <span className="font-semibold">
                    {t("import_connectors.skipped", { defaultValue: "Skipped:" })}
                  </span>
              ...
              {result.errors.length > 0 && (
                <div>
                  <span className="font-semibold">
                    {t("import_connectors.failed", { defaultValue: "Failed:" })}
                  </span>
                  <ul className="ml-4 list-disc">
          ...
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={handleClose} disabled={isImporting}>
              {t("import_connectors.close", { defaultValue: "Close" })}
            </Button>
            <Button onClick={handleImport} disabled={isImporting}>
              {isImporting
                ? t("import_connectors.importing", { defaultValue: "Importing..." })
                : t("import_connectors.import", { defaultValue: "Import" })}
            </Button>
          </div>
        </div>
      </DialogContent>
