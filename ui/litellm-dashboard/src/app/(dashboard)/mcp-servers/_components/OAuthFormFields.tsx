...
const UpstreamResourceField: React.FC = () => {
  ...
  return (
    <MountedFormField
      label={
        <FieldLabel
          label={t("mcp:oauth_fields.resource_indicator", "Resource Indicator (optional)")}
          tooltip={UPSTREAM_RESOURCE_TOOLTIP}
        />
      }
...
const OAuthFormFields: React.FC<OAuthFormFieldsProps> = ({
  ...
  const oauthFlowItems = [
    { value: OAUTH_FLOW.M2M, label: t("mcp:oauth_fields.m2m_title", "Machine-to-Machine (M2M)") },
    { value: OAUTH_FLOW.INTERACTIVE, label: t("mcp:oauth_fields.pkce_title", "Interactive (PKCE)") },
  ];
  ...
  return (
    <>
      <MountedFormField
        label={
          <FieldLabel
            label={t("mcp:oauth_fields.oauth_flow_type", "OAuth Flow Type")}
            tooltip={t(
              "mcp:oauth_fields.oauth_flow_type_tooltip",
        ...
        {(control) => (
          <Select {...selectControl<string>(control)} items={oauthFlowItems}>
            <SelectTrigger {...selectTriggerControl(control)} className="w-full rounded-lg">
              <SelectValue placeholder={t("mcp:oauth_fields.select_flow_placeholder", "Select OAuth flow")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value={OAUTH_FLOW.M2M}>
                <div>
                  <span className="font-medium">
                    {t("mcp:oauth_fields.m2m_title", "Machine-to-Machine (M2M)")}
                  </span>
                  <span className="ml-2 text-xs text-muted-foreground">
                    {t("mcp:oauth_fields.m2m_desc", "server-to-server, no user interaction")}
                  </span>
                </div>
              </SelectItem>
              <SelectItem value={OAUTH_FLOW.INTERACTIVE}>
                <div>
                  <span className="font-medium">
                    {t("mcp:oauth_fields.pkce_title", "Interactive (PKCE)")}
                  </span>
                  <span className="ml-2 text-xs text-muted-foreground">
                    {t("mcp:oauth_fields.pkce_desc", "browser-based user authorization")}
                  </span>
                </div>
              </SelectItem>
            </SelectContent>
      ...
      {isM2M ? (
        <>
          <MountedFormField
            label={
              <FieldLabel
                label={t("mcp:oauth_fields.client_id", "Client ID")}
          ...
          <MountedFormField
            label={
              <FieldLabel
                label={t("mcp:oauth_fields.client_secret", "Client Secret")}
          ...
          <MountedFormField
            label={
              <FieldLabel
                label={t("mcp:oauth_fields.token_url", "Token URL")}
          ...
          <MountedFormField
            label={
              <FieldLabel
                label={t("mcp:oauth_fields.scopes_optional", "Scopes (optional)")}
          ...
          >
            {(control) => (
              <MultiSelect
                {...tagsControl(control)}
                placeholder={t("mcp:oauth_fields.add_scopes_placeholder", "Add scopes")}
        ...
        <>
          <MountedFormField
            label={
              <span className="flex items-center justify-between w-full">
                <FieldLabel
                  label={t("mcp:oauth_fields.client_id_optional", "Client ID (optional)")}
                ...
                {docsUrl && (
                  ...
                  >
                    {t("mcp:oauth_fields.create_oauth_app", "Create OAuth App →")}
                  </a>
                )}
              </span>
            }
          ...
          <MountedFormField
            label={
              <FieldLabel
                label={t("mcp:oauth_fields.client_secret_optional", "Client Secret (optional)")}
                tooltip="Provide only if your MCP server cannot handle dynamic client registration."
              />
            }
            name={["credentials", "client_secret"]}
          ...
          <MountedFormField
            label={
              <FieldLabel
                label={t("mcp:oauth_fields.issuer_optional", "Issuer (optional)")}
          ...
          <MountedFormField
            label={
              <FieldLabel
                label={t("mcp:oauth_fields.auth_url_optional", "Authorization URL (optional)")}
          ...
          <MountedFormField
            label={
              <FieldLabel
                label={t("mcp:oauth_fields.token_url_optional", "Token URL (optional)")}
          ...
          <MountedFormField
            label={
              <FieldLabel
                label={t("mcp:oauth_fields.registration_url_optional", "Registration URL (optional)")}
          ...
          <MountedFormField
            label={
              <FieldLabel
                label={t("mcp:oauth_fields.token_validation_json", "Token Validation Rules (optional)")}
          ...
          <MountedFormField
            label={
              <FieldLabel
                label={t("mcp:oauth_fields.token_storage_ttl", "Token Storage TTL (seconds, optional)")}
            ...
            <div className="rounded-lg border border-dashed border-border p-4 space-y-2">
              ...
              >
                {oauthFlow.status === "authorizing"
                  ? t("mcp:oauth_fields.waiting_auth", "Waiting for authorization...")
                  : oauthFlow.status === "exchanging"
                    ? t("mcp:oauth_fields.exchanging_code", "Exchanging authorization code...")
                    : t("mcp:oauth_fields.authorize_fetch", "Authorize & Fetch Token")}
              </Button>
              ...
              {oauthFlow.status === "success" && oauthFlow.tokenResponse?.access_token && (
                <p className="text-sm text-success">
                  {t(
                    "mcp:oauth_fields.token_fetched",
                    `Token fetched. Expires in ${oauthFlow.tokenResponse.expires_in ?? "?"} seconds.`,
                    { expiresIn: oauthFlow.tokenResponse.expires_in ?? "?" },
                  )}
                </p>
              )}
            </div>
