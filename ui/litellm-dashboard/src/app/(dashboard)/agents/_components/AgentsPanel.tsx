...
const AgentsPanel: React.FC<AgentsPanelProps> = ({ accessToken, userRole, teams }) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(
        t("agents:deleted_success", {
          name: agentToDelete.name,
          defaultValue: `Agent "${agentToDelete.name}" deleted successfully`,
        }),
      );
      await refetchAgents(healthCheckEnabled);
    } catch (error) {
      console.error("Error deleting agent:", error);
      toast.fromError(t("agents:delete_failed", { defaultValue: "Failed to delete agent" }));
    } finally {
      setIsDeleting(false);
    ...
    <div className="w-full mx-auto flex-auto overflow-y-auto m-8 p-2">
      <div className="flex flex-col gap-2 mb-4">
        ...
        <Alert className="mb-3">
          <Info />
          <AlertTitle>{t("agents:why_keys_title", { defaultValue: "Why do agents need keys?" })}</AlertTitle>
          <AlertDescription>
            {t("agents:why_keys_desc", {
              defaultValue:
        ...
        {isAdmin && (
          <div className="mt-2 flex items-center gap-4">
            <Button onClick={handleAddAgent} disabled={!accessToken}>
              <Plus />
              {t("agents:add_new_agent", { defaultValue: "Add New Agent" })}
            </Button>
          </div>
        )}
      </div>
      ...
      {agentToDelete && (
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>{t("agents:delete_agent", { defaultValue: "Delete Agent" })}</AlertDialogTitle>
              <AlertDialogDescription>
                {t("agents:delete_confirm", {
                  name: agentToDelete.name,
                  defaultValue: `Are you sure you want to delete agent: ${agentToDelete.name}? This action cannot be undone.`,
                })}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
