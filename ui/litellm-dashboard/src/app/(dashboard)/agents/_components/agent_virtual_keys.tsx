  ...
  return (
    <div className="mt-6">
      <h4 className="text-base font-semibold text-foreground">
        {t("virtual_keys.title", { defaultValue: "Virtual Keys" })}
      </h4>
      {isLoading ? (
        <p className="mt-2 text-sm text-muted-foreground">
          {t("virtual_keys.loading", { defaultValue: "Loading keys..." })}
        </p>
      ) : keys.length === 0 ? (
        <p className="mt-2 text-sm text-muted-foreground">
          {t("virtual_keys.no_keys", { defaultValue: "No virtual key assigned to this agent." })}
        </p>
      ) : (
          ...
          {keys.map((key) => (
            <div key={key.token} className="flex items-center gap-3 rounded-sm border border-border px-3 py-2">
              ...
              <span className="text-sm font-medium text-foreground">
                {key.key_alias || t("virtual_keys.unnamed_key", { defaultValue: "Unnamed key" })}
              </span>
