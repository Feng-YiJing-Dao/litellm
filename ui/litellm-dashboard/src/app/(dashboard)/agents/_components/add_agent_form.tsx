  ...
  if (agentType === CUSTOM_AGENT_TYPE) {
    return (
      <span className="flex items-center gap-2">
        ...
        <span>{t("create_modal.custom_other", { defaultValue: "Custom / Other" })}</span>
      </span>
    );
  }
...
const StepProgress: React.FC<{ current: number }> = ({ current }) => {
  const { t } = useTranslation("agents");
  const stepTitles = [
    t("create_modal.steps.configure", { defaultValue: "Configure" }),
    t("create_modal.steps.entitlements", { defaultValue: "Entitlements" }),
    t("create_modal.steps.governance", { defaultValue: "Governance" }),
    t("create_modal.steps.management", { defaultValue: "Agent Management" }),
    t("create_modal.steps.ready", { defaultValue: "Ready" }),
  ];
    ...
    <ol aria-label="Agent creation steps" className="mb-8 flex items-center">
      {stepTitles.map((title, index) => (
        ...
        >
          ...
          {index < stepTitles.length - 1 && <span aria-hidden="true" className="mx-2 h-px flex-1 bg-border" />}
        </li>
      ))}
    </ol>
  );
...
const AddAgentForm: React.FC<AddAgentFormProps> = ({ visible, onClose, accessToken, onSuccess, teams }) => {
  ...
  const handleCreateAgent = async () => {
    if (!accessToken) {
      toast.error(t("create_modal.no_token_error", { defaultValue: "No access token available" }));
      return;
    }
    ...
    try {
      ...
      if (!agentData) {
        toast.error(t("create_modal.build_data_failed", { defaultValue: "Failed to build agent data" }));
        setIsSubmitting(false);
        return;
      }
      ...
      if (keyAssignOption === "create_new" && newKeyName) {
        ...
        if (!selectedExistingKey) {
          toast.error(t("create_modal.select_existing_key_error", { defaultValue: "Please select an existing key to assign" }));
          setIsSubmitting(false);
          return;
        }
      ...
      toast.error(
        errorMessage
          ? t("create_modal.create_failed_with_msg", { defaultValue: "Failed to create agent: {{msg}}", msg: errorMessage })
          : t("create_modal.create_failed", { defaultValue: "Failed to create agent" })
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const renderEntitlementsStep = () => (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        {t("create_modal.entitlements_desc", {
          defaultValue:
      ...
      <FieldGroup>
        <AgentFormField
          ...
          label={labelWithHint(
            t("create_modal.allowed_models", { defaultValue: "Allowed Models" }),
            t("create_modal.allowed_models_hint", {
        ...
        >
          {({ id, value, onChange }) => (
            <AgentTagsInput
              ...
              placeholder={
                loadingModels
                  ? t("create_modal.loading_models", { defaultValue: "Loading models..." })
                  : t("create_modal.select_models_placeholder", { defaultValue: "Select models (leave empty for all)" })
              }
        ...
        <AgentFormField
          ...
          label={labelWithHint(
            t("create_modal.allowed_agents", { defaultValue: "Allowed Agents (Sub-Agents)" }),
            t("create_modal.allowed_agents_hint", {
              defaultValue: "Restrict which other agents this agent can invoke as sub-agents. Leave empty to allow all.",
            }),
          )}
        >
          {({ id, value, onChange }) => (
            <AgentMultiSelect
              ...
              placeholder={
                loadingAgents
                  ? t("create_modal.loading_agents", { defaultValue: "Loading agents..." })
                  : t("create_modal.select_agents_placeholder", { defaultValue: "Select agents (leave empty for all)" })
              }
        ...
        <AgentFormField
          ...
          label={labelWithHint(
            t("create_modal.allowed_mcp_servers", { defaultValue: "Allowed MCP Servers" }),
            t("create_modal.allowed_mcp_servers_hint", {
        ...
        >
          {({ value, onChange }) => (
            <MCPServerSelector
              ...
              placeholder={t("create_modal.select_mcp_placeholder", {
    ...
    <div className="space-y-6">
      <div>
        <h4 className="mb-3 text-sm font-medium text-foreground">
          {t("create_modal.tracing_title", { defaultValue: "Tracing" })}
        </h4>
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-foreground">
                {t("create_modal.require_trace_inbound", {
                  defaultValue: "Require x-litellm-trace-id on calls TO this agent",
                })}
              </span>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("create_modal.require_trace_inbound_desc", {
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-foreground">
                {t("create_modal.require_trace_outbound", {
                  defaultValue: "Require x-litellm-trace-id on calls BY this agent",
                })}
              </span>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("create_modal.require_trace_outbound_desc", {
                  defaultValue:
      ...
      <div>
        <h4 className="mb-3 text-sm font-medium text-foreground">
          {t("create_modal.budgets_rate_limits", { defaultValue: "Budgets & Rate Limits" })}
        </h4>
        <div className="space-y-4">
          {!requireTraceIdOutbound && (
            <div className="rounded-lg border border-warning/20 bg-warning/10 p-3 text-sm text-warning">
              {t("create_modal.enable_trace_outbound_warning", {
                defaultValue:
          ...
          <div className="text-sm font-medium text-foreground">
            {t("create_modal.session_budgets", { defaultValue: "Session Budgets" })}
          </div>
          <div className="grid grid-cols-2 gap-4">
            <Field className="gap-1">
              <FieldLabel htmlFor="agent-max-iterations">
                {t("create_modal.max_iterations", { defaultValue: "Max Iterations" })}
              </FieldLabel>
              <Input
              ...
              />
              <p className="mt-1 text-xs text-muted-foreground">
                {t("create_modal.max_iterations_desc", { defaultValue: "Hard cap on LLM calls per session" })}
              </p>
            </Field>
            <Field className="gap-1">
              <FieldLabel htmlFor="agent-max-budget-per-session">
                {t("create_modal.max_budget_per_session", { defaultValue: "Max Budget Per Session ($)" })}
              </FieldLabel>
              <Input
              ...
              />
              <p className="mt-1 text-xs text-muted-foreground">
                {t("create_modal.max_budget_desc", { defaultValue: "Max spend per trace before returning 429" })}
              </p>
            </Field>
          </div>
          ...
          <div className="text-sm font-medium text-foreground">
            {t("create_modal.agent_rate_limits", { defaultValue: "Agent Rate Limits" })}
          </div>
          <p className="text-xs text-muted-foreground">
            {t("create_modal.agent_rate_limits_desc", {
          ...
          <div className="mt-4 text-sm font-medium text-foreground">
            {t("create_modal.per_session_rate_limits", { defaultValue: "Per-Session Rate Limits" })}
          </div>
          <p className="text-xs text-muted-foreground">
            {t("create_modal.per_session_rate_limits_desc", {
      ...
      <div>
        <h4 className="mb-3 text-sm font-medium text-foreground">
          {t("create_modal.guardrails_title", { defaultValue: "Guardrails" })}
        </h4>
        <p className="mb-3 text-xs text-muted-foreground">
          {t("create_modal.guardrails_desc", {
            defaultValue: "Apply guardrails to this agent. Selected guardrails will run on all calls made by this agent.",
          })}
        </p>
    ...
    <>
      <Field className="gap-1">
        <FieldLabel htmlFor="agent-type">
          {labelWithHint(
            t("create_modal.agent_type", { defaultValue: "Agent Type" }),
            t("create_modal.agent_type_hint", { defaultValue: "Select the type of agent you want to create" }),
          )}
        </FieldLabel>
        <Select value={agentType} onValueChange={(value) => value !== null && handleAgentTypeChange(value)}>
          ...
          <SelectContent className="p-1">
            ...
            <div className="mb-1 px-2 text-xs font-medium tracking-wide text-muted-foreground uppercase">
              {t("create_modal.not_listed", { defaultValue: "Not listed?" })}
            </div>
                ...
                <span className="block">
                  <span className="flex items-center gap-2">
                    <span className="font-medium text-warning">
                      {t("create_modal.custom_other", { defaultValue: "Custom / Other" })}
                    </span>
      ...
      <div className="mt-4">
        {agentType === CUSTOM_AGENT_TYPE ? (
          <FieldGroup>
            <AgentFormField
              name="agent_name"
              label={t("create_modal.agent_name", { defaultValue: "Agent Name" })}
              rules={{ required: t("create_modal.agent_name_required", { defaultValue: "Please enter an agent name" }) }}
            >
              {({ value, onChange, ref, ...control }) => (
                <Input
                  {...control}
                  ref={ref}
                  placeholder={t("create_modal.agent_name_placeholder", { defaultValue: "e.g. my-custom-agent" })}
            ...
            <AgentFormField
              name="description"
              label={t("create_modal.description", { defaultValue: "Description" })}
            >
              {({ value, onChange, ref, ...control }) => (
                <Textarea
                  {...control}
                  ref={ref}
                  rows={3}
                  placeholder={t("create_modal.description_placeholder", { defaultValue: "Describe what this agent does…" })}
                  value={typeof value === "string" ? value : ""}
                  onChange={onChange}
                />
              )}
          ...
          <>
            ...
            {selectedAgentTypeInfo.credential_fields.length > 0 && (
              <div className="mt-4 rounded-lg border border-border p-4">
                <h4 className="mb-3 text-sm font-medium text-foreground">
                  {selectedAgentTypeInfo.agent_type_display_name} {t("create_modal.settings_suffix", { defaultValue: "Settings" })}
                </h4>
                <FieldGroup>
    ...
    return (
      <div>
        ...
        <AgentFormField
          name="team_id"
          label={labelWithHint(
            t("create_modal.assign_team", { defaultValue: "Assign to Team" }),
            t("create_modal.assign_team_hint", {
              defaultValue:
        ...
        >
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex flex-1 items-start gap-3">
                <RadioGroupItem
                  value="create_new"
                  aria-label={t("create_modal.create_new_key", { defaultValue: "Create a new key for this agent" })}
                />
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <Key className="size-4 text-info" />
                    <span className="font-medium text-foreground">
                      {t("create_modal.create_new_key", { defaultValue: "Create a new key for this agent" })}
                    </span>
                  </div>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {t("create_modal.create_new_key_desc", {
                    ...
                    <div className="mt-3 space-y-3" onClick={(e) => e.stopPropagation()}>
                      <Field className="gap-1">
                        <FieldLabel htmlFor="agent-new-key-name">
                          {t("create_modal.key_name", { defaultValue: "Key Name" })}
                        </FieldLabel>
                        <Input
                          id="agent-new-key-name"
                          value={newKeyName}
                          onChange={(e) => setNewKeyName(e.target.value)}
                          placeholder={t("create_modal.key_name_placeholder", { defaultValue: "e.g. my-agent-key" })}
                        />
                      </Field>
                    </div>
                  )}
              ...
              <StatusBadge tone="success" label={t("create_modal.recommended", { defaultValue: "Recommended" })} />
            </div>
          </div>
          ...
          >
            <div className="flex items-start gap-3">
              <RadioGroupItem
                value="existing_key"
                aria-label={t("create_modal.assign_existing_key", { defaultValue: "Assign an existing key" })}
              />
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <Key className="size-4 text-muted-foreground" />
                  <span className="font-medium text-foreground">
                    {t("create_modal.assign_existing_key", { defaultValue: "Assign an existing key" })}
                  </span>
                </div>
                <p className="mt-1 text-sm text-muted-foreground">
                  {t("create_modal.assign_existing_key_desc", {
                ...
                {keyAssignOption === "existing_key" && (
                  <div className="mt-3" onClick={(e) => e.stopPropagation()}>
                    <SearchSelect
                      ...
                      placeholder={
                        loadingKeys
                          ? t("create_modal.loading_keys", { defaultValue: "Loading keys…" })
                          : t("create_modal.search_key_placeholder", { defaultValue: "Search by key name…" })
                      }
        ...
        <div className="mt-4 text-center">
          ...
          >
            {t("create_modal.skip_key", { defaultValue: "Skip for now — I'll assign a key later" })}
          </button>
        </div>
      </div>
    );
  ...
  const renderReadyStep = () => (
    <div className="py-6 text-center">
      ...
      <h3 className="mb-2 text-xl font-semibold text-foreground">
        {t("create_modal.agent_created", { defaultValue: "Agent Created!" })}
      </h3>
      ...
      {assignedKeyAlias && (
        <p className="mt-2 text-sm text-muted-foreground">
          {t("create_modal.key_assigned_notice", {
            defaultValue: "Key {{alias}} has been assigned to this agent.",
            alias: assignedKeyAlias,
          })}
        </p>
      )}
      {!createdKeyValue && !assignedKeyAlias && keyAssignOption === "skip" && (
        <p className="mt-2 text-sm text-muted-foreground">
          {t("create_modal.no_key_assigned", {
        ...
        <DialogHeader>
          <div className="flex items-center space-x-3 border-b border-border pb-4">
            ...
            <DialogTitle className="text-xl font-semibold text-foreground">
              {t("create_modal.title", { defaultValue: "Add New Agent" })}
            </DialogTitle>
          </div>
        </DialogHeader>
        <TooltipProvider>
          <div className="mt-4">
            ...
            <div className="mt-6 flex items-center justify-between border-t border-border pt-6">
              <div>
                {currentStep > 0 && currentStep < 4 && (
                  <Button type="button" variant="outline" onClick={handleBack}>
                    {t("create_modal.back", { defaultValue: "← Back" })}
                  </Button>
                )}
              </div>
              <div className="flex gap-3">
                ...
                {currentStep < 3 && (
                  <Button onClick={handleNext}>{t("create_modal.next", { defaultValue: "Next →" })}</Button>
                )}
                {currentStep === 3 && (
                  <Button disabled={isSubmitting} aria-busy={isSubmitting} onClick={handleCreateAgent}>
                    ...
                    {isSubmitting
                      ...
                      : t("create_modal.create_agent_btn", { defaultValue: "Create Agent →" })}
                  </Button>
                )}
                {currentStep === 4 && (
                  <Button onClick={handleClose}>{t("create_modal.done", { defaultValue: "Done" })}</Button>
                )}
              </div>
            </div>
          </div>
