...
const ClaudeCodePluginsPanel: React.FC<ClaudeCodePluginsPanelProps> = ({ accessToken, userRole }) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(t("skills:delete_modal.success", "Skill \"{{name}}\" deleted successfully", { name: pluginToDelete.displayName }));
      fetchPlugins();
    } catch (error) {
      console.error("Error deleting skill:", error);
      toast.error(t("skills:delete_modal.failed", "Failed to delete skill"));
    } finally {
      setIsDeleting(false);
    ...
    <div className="w-full mx-auto flex-auto overflow-y-auto m-8 p-2">
      {selectedSkill ? (
        ...
        <>
          <div className="flex flex-col gap-2 mb-4">
            <h1 className="text-2xl font-bold">{t("skills:title", "Skills")}</h1>
            <p className="text-sm text-muted-foreground">
              {t("skills:subtitle", "Register Claude Code skills. Published skills appear in the Skill Hub for all users and are served via")}{" "}
      ...
      {pluginToDelete && (
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>{t("skills:delete_modal.title", "Delete Skill")}</AlertDialogTitle>
              <AlertDialogDescription>
                {t("skills:delete_modal.confirm_prompt", "Are you sure you want to delete skill: ")}<strong>{pluginToDelete.displayName}</strong>?
              </AlertDialogDescription>
              <p className="text-sm text-muted-foreground">{t("skills:delete_modal.cannot_be_undone", "This action cannot be undone.")}</p>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>{t("skills:delete_modal.cancel", "Cancel")}</AlertDialogCancel>
              <Button variant="destructive" onClick={handleDeleteConfirm} disabled={isDeleting}>
                {t("skills:delete_modal.delete", "Delete")}
              </Button>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
