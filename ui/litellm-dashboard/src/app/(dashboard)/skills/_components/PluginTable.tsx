...
function EmptyState({ t }: { t?: (key: any, options?: any) => any }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{tr("skills:table.no_skills_found", { defaultValue: "No skills found" })}</div>
      <div className="text-sm text-muted-foreground">{tr("skills:table.add_one_started", { defaultValue: "Add one to get started." })}</div>
    </div>
  );
}
...
const PluginTable: React.FC<PluginTableProps> = ({ pluginsList, isLoading, onDeleteClick, isAdmin, onPluginClick }) => {
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      loadingMessage={t("skills:table.loading", "Loading skills…")}
