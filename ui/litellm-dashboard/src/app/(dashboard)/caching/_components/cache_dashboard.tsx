...
const CacheDashboard: React.FC<CachePageProps> = ({ accessToken, token, userRole, userID, premiumUser }) => {
  ...
  const statCards = [
    { label: t("caching:stats.hit_ratio", { defaultValue: "Cache Hit Ratio" }), value: `${hasRequests ? totals.cache_hit_ratio.toFixed(2) : "0"}%` },
    { label: t("caching:stats.cache_hits", { defaultValue: "Cache Hits" }), value: valueFormatterNumbers(totals?.cache_hits ?? 0) },
    { label: t("caching:stats.cached_tokens", { defaultValue: "Cached Completion Tokens" }), value: valueFormatterNumbers(totals?.cached_completion_tokens ?? 0) },
  ];
      ...
      <div className="mt-2 flex w-full items-center justify-between border-b">
        <TabsList variant="line" className="h-auto rounded-none p-0">
          <TabsTrigger value="analytics" className="flex-none rounded-none px-4 py-2">
            {t("caching:tabs.analytics", { defaultValue: "Cache Analytics" })}
          </TabsTrigger>
          <TabsTrigger value="health" className="flex-none rounded-none px-4 py-2">
            {t("caching:tabs.health", { defaultValue: "Cache Health" })}
          </TabsTrigger>
          <TabsTrigger value="settings" className="flex-none rounded-none px-4 py-2">
            {t("caching:tabs.settings", { defaultValue: "Cache Settings" })}
          </TabsTrigger>
          <TabsTrigger value="coordination" className="flex-none rounded-none px-4 py-2">
            {t("caching:tabs.coordination", { defaultValue: "Coordination Redis" })}
          </TabsTrigger>
        </TabsList>

        <div className="flex items-center space-x-2">
          {lastRefreshed && <p className="text-sm text-muted-foreground">{t("caching:last_refreshed", { time: lastRefreshed, defaultValue: `Last Refreshed: ${lastRefreshed}` })}</p>}
      ...
      <TabsContent value="analytics" keepMounted>
        <Card>
          <CardContent>
            ...
            <div className="mt-4 grid grid-cols-1 items-center gap-4 md:grid-cols-[1fr_1fr_auto]">
              ...
              >
                <ComboboxChips render={<div ref={anchor1} />}>
                  ...
                  <ComboboxChipsInput placeholder={t("caching:select_keys", { defaultValue: "Select Virtual Keys" })} />
                </ComboboxChips>
                <ComboboxContent anchor={anchor1}>
                  <ComboboxEmpty>{t("caching:no_keys_found", { defaultValue: "No virtual keys found" })}</ComboboxEmpty>
                  <ComboboxList>
                    {(key: string) => (
              ...
              >
                <ComboboxChips render={<div ref={anchor2} />}>
                  ...
                  <ComboboxChipsInput placeholder={t("caching:select_models", { defaultValue: "Select Models" })} />
                </ComboboxChips>
                <ComboboxContent anchor={anchor2}>
                  <ComboboxEmpty>{t("caching:no_models_found", { defaultValue: "No models found" })}</ComboboxEmpty>
                  <ComboboxList>
                    {(model: string) => (
            ...
            <Card className="mt-4">
              <CardHeader>
                <CardTitle className="text-base font-semibold">{t("caching:charts.hits_vs_requests", { defaultValue: "Cache Hits vs API Requests" })}</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-muted-foreground">
                  {t("caching:charts.hits_desc", { defaultValue: "Click a red failed-requests segment to see which error codes caused those failures." })}
                </p>
                <BarChart
                  data={chartData}
                  stack={true}
            ...
            <Card className="mt-6">
              <CardHeader>
                <CardTitle className="text-base font-semibold">
                  {t("caching:charts.tokens_title", { defaultValue: "Cached Completion Tokens vs Generated Completion Tokens" })}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <BarChart
