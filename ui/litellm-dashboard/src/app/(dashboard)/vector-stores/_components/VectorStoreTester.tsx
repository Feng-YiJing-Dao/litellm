...
export const VectorStoreTester: React.FC<VectorStoreTesterProps> = ({ vectorStoreId, accessToken, className = "" }) => {
  ...
  const handleSearch = async () => {
    if (!query.trim()) {
      toast.warning(t("vector_stores.tester.search_query_required", { defaultValue: "Please enter a search query" }));
      return;
    }
  ...
  const clearHistory = () => {
    setSearchHistory([]);
    setExpandedResults({});
    toast.success(t("vector_stores.tester.history_cleared", { defaultValue: "Search history cleared" }));
  };
        ...
        <div className="flex items-center justify-between border-b p-4">
          <div className="flex items-center">
            ...
            <h4 className="text-base font-medium text-foreground">
              {t("vector_stores.tester.title", { defaultValue: "Test Vector Store" })}
            </h4>
          </div>
          {searchHistory.length > 0 && (
            <Button variant="outline" size="sm" onClick={clearHistory}>
              {t("vector_stores.tester.clear_history", { defaultValue: "Clear History" })}
            </Button>
          )}
        </div>
        ...
        <div className="flex-1 overflow-auto p-4 pb-0">
          {searchHistory.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
              ...
              <p className="text-sm">
                {t("vector_stores.tester.empty_state", {
                  ...
                  <div className="text-right">
                    <div className="inline-block max-w-[80%] rounded-lg bg-muted p-3 shadow-xs ring-1 ring-foreground/10">
                      <div className="mb-1 flex items-center gap-2">
                        <strong className="text-sm">{t("vector_stores.tester.query_label", { defaultValue: "Query" })}</strong>
                    ...
                    <div className="inline-block max-w-[80%] rounded-lg bg-card p-3 shadow-xs ring-1 ring-foreground/10">
                      <div className="mb-2 flex items-center gap-2">
                        ...
                        <strong className="text-sm">
                          {t("vector_stores.tester.results_label", { defaultValue: "Vector Store Results" })}
                        </strong>
                        {entry.response && (
                          <span className="rounded-sm bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                            {t("vector_stores.tester.results_count", {
                              count: entry.response.data?.length || 0,
                              defaultValue: `${entry.response.data?.length || 0} results`,
                            })}
                          </span>
                        )}
                      </div>
                      ...
                      {entry.response && entry.response.data && entry.response.data.length > 0 ? (
                                ...
                                >
                                  <div className="flex items-center">
                                    ...
                                    <span className="text-sm font-medium">
                                      {t("vector_stores.tester.result_title", {
                                  ...
                                  <span className="rounded-sm bg-muted px-2 py-1 text-xs text-foreground">
                                    {t("vector_stores.tester.score", {
                                      score: result.score.toFixed(4),
                                      defaultValue: `Score: ${result.score.toFixed(4)}`,
                                    })}
                                  </span>
                                </div>
                                      ...
                                      result.content.map((content, contentIndex) => (
                                        <div key={contentIndex} className="mb-3">
                                          <div className="mb-1 text-xs text-muted-foreground">
                                            {t("vector_stores.tester.content_type", {
                                              type: content.type,
                                              defaultValue: `Content (${content.type})`,
                                            })}
                                          </div>
                                    ...
                                    {(result.file_id || result.filename || result.attributes) && (
                                      <div className="mt-3 border-t pt-3">
                                        <div className="mb-2 text-xs font-medium text-muted-foreground">
                                          {t("vector_stores.tester.metadata", { defaultValue: "Metadata" })}
                                        </div>
                                        <div className="space-y-2 text-xs">
                                          {result.file_id && (
                                            <div className="rounded-sm bg-muted/50 p-2">
                                              <span className="font-medium">
                                                {t("vector_stores.tester.file_id", { defaultValue: "File ID:" })}{" "}
                                              </span>
                                              {result.file_id}
                                            </div>
                                          )}
                                          {result.filename && (
                                            <div className="rounded-sm bg-muted/50 p-2">
                                              <span className="font-medium">
                                                {t("vector_stores.tester.filename", { defaultValue: "Filename:" })}{" "}
                                              </span>
                                              {result.filename}
                                            </div>
                                          )}
                                          {result.attributes && Object.keys(result.attributes).length > 0 && (
                                            <div className="rounded-sm bg-muted/50 p-2">
                                              <span className="mb-1 block font-medium">
                                                {t("vector_stores.tester.attributes", { defaultValue: "Attributes:" })}
                                              </span>
                        ...
                        >
                          {entry.error
                            ? t("vector_stores.tester.search_failed", {
                                error: entry.error,
                                defaultValue: `Search failed: ${entry.error}`,
                              })
                            : t("vector_stores.tester.no_results", { defaultValue: "No results found" })}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="flex items-end space-x-2">
            <div className="flex-1">
              <Textarea
                ...
                placeholder={t("vector_stores.tester.input_placeholder", {
            ...
            <Button onClick={handleSearch} disabled={isLoading || !query.trim()}>
              ...
              {t("vector_stores.tester.search_btn", { defaultValue: "Search" })}
            </Button>
          </div>
        </div>
      </div>
