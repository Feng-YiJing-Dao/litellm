...
function EmptyState({ t }: { t?: (key: any, options?: any) => any }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{tr("policies:table.no_policies", { defaultValue: "No policies found" })}</div>
      <div className="text-sm text-muted-foreground">
        {tr("policies:table.no_policies_desc", { defaultValue: "Create a policy to bundle guardrails and apply them across teams." })}
      </div>
    </div>
  );
}
...
const PolicyTable: React.FC<PolicyTableProps> = ({
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      loadingMessage={t("policies:table.loading_policies", "Loading policies…")}
