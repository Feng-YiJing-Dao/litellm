...
function EmptyState({ t }: { t?: (key: any, options?: any) => any }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{tr("policies:table.no_attachments", { defaultValue: "No attachments found" })}</div>
      <div className="text-sm text-muted-foreground">
        {tr("policies:table.no_attachments_desc", { defaultValue: "Attach a policy to teams, keys, models, or tags to control where it applies." })}
      </div>
    </div>
  );
}
...
const AttachmentTable: React.FC<AttachmentTableProps> = ({
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      loadingMessage={t("policies:table.loading_attachments", "Loading attachments…")}
