      ...
      <DialogContent className="sm:max-w-175">
        ...
        <div className="space-y-4 py-4">
          ...
          {hasEnrichment && (
            <>
              <div>
                <label className="mb-2 block text-sm font-medium">{t("policies:templates.competitor_discovery", "Competitor Discovery")}</label>
                <RadioGroup
                ...
                >
                  <label className="flex cursor-pointer items-center justify-center gap-2 rounded-md border border-input px-3 py-2 text-sm">
                    <RadioGroupItem value="ai" />{t("policies:templates.use_ai", "✨ Use AI")}
                  </label>
                  <label className="flex cursor-pointer items-center justify-center gap-2 rounded-md border border-input px-3 py-2 text-sm">
                    <RadioGroupItem value="manual" />
                    {t("policies:templates.enter_manually", "Enter Manually")}
                  </label>
                </RadioGroup>
              </div>
              ...
              <div>
                <label className="mb-1 block text-sm font-medium">
                  {t("policies:templates.your_brand_name", "Your Brand Name")}
                  <span className="ml-1 text-destructive">*</span>
                </label>
                <Input
                  placeholder={t("policies:templates.brand_placeholder", "e.g. Acme Airlines")}
              ...
              {competitorMode === "ai" && (
                <>
                  <div>
                    <label className="mb-1 block text-sm font-medium">
                      {t("policies:templates.select_model", "Select Model")}
                      <span className="ml-1 text-destructive">*</span>
                    </label>
                    <SearchSelect
                      ...
                      placeholder={isLoadingModels ? t("policies:templates.loading_models", "Loading models...") : t("policies:templates.select_model_placeholder", "Select a model to generate names")}
                      emptyText={t("policies:templates.no_models_found", "No models found")}
                  ...
                  <Button
                    ...
                    className="w-full"
                  >
                    {isGenerating ? t("policies:templates.generating_names", "✨ Generating names...") : t("policies:templates.generate_competitor_names", "✨ Generate Competitor Names")}
                  </Button>
                </>
              )}
              ...
              <div>
                <label className="mb-1 block text-sm font-medium">
                  {t("policies:templates.competitor_names", "Competitor Names")}
                ...
                <div className="flex flex-wrap items-center gap-1.5 rounded-md border border-input p-2">
                  ...
                  <input
                    ...
                    placeholder={t("policies:templates.competitor_input_placeholder", "Type a name and press Enter to add")}
                    value={tagDraft}
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("policies:templates.competitor_input_tip", "Type a name and press Enter to add. Click ✕ to remove.")}
                </p>
                {statusMessage && (
                ...
                {Object.keys(variationsMap).length > 0 && !statusMessage && (
                  <p className="mt-1 text-xs text-success">
                    {t("policies:templates.variations_generated", "✓ {{count}} alternate spellings & variations auto-generated for guardrail matching", { count: Object.values(variationsMap).flat().length })}
                  </p>
                )}
              </div>
              ...
              {competitorMode === "ai" && hasGenerated && competitorTags.length > 0 && (
                <div>
                  <label className="mb-1 block text-sm font-medium">{t("policies:templates.refine_list", "Refine List")}</label>
                  <div className="flex gap-2">
                    <Input
                      placeholder={t("policies:templates.refine_placeholder", "e.g. add 10 more from Asia, increase to 50 total...")}
                    ...
                    <Button onClick={handleRefine} disabled={!refinementInput.trim() || isRefining} size="sm">
                      {isRefining ? "..." : t("policies:templates.send", "Send")}
                    </Button>
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t("policies:templates.refine_tip", "Give instructions to add, remove, or change competitors. Press Enter to send.")}
                  </p>
                </div>
              )}
            </>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={!canContinue || isLoading}>
            {isLoading ? t("policies:templates.creating_guardrails", "Creating guardrails...") : t("policies:templates.continue", "Continue")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
