...
const GuardrailSelectionModal: React.FC<GuardrailSelectionModalProps> = ({
  ...
  const guardrailsInfo: GuardrailInfo[] = (template?.guardrailDefinitions || []).map((def: any) => ({
    ...
    description: def.guardrail_info?.description || t("policies:templates.no_description_available", "No description available"),
    ...
    <Dialog open={visible} onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className="sm:max-w-175">
        <DialogHeader>
          ...
          <DialogDescription>{t("policies:templates.review_guardrails", "Review and select guardrails to create for this template")}</DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4">
          ...
          <div className="mb-4 flex items-center gap-4 rounded-lg border border-border bg-muted p-3">
            ...
            <div className="flex-1">
              <div className="text-sm">
                <span className="font-medium">{t("policies:templates.total_guardrails", "{{count}} total guardrails", { count: guardrailsInfo.length })}</span>
                <span className="mx-2 text-muted-foreground">•</span>
                <span className="font-medium text-success">{t("policies:templates.new_guardrails", "{{count}} new", { count: newGuardrailsCount })}</span>
                {existingCount > 0 && (
                  <>
                    <span className="mx-2 text-muted-foreground">•</span>
                    <span className="text-muted-foreground">{t("policies:templates.already_exist_count", "{{count}} already exist", { count: existingCount })}</span>
                  </>
                )}
              </div>
            </div>
            {newGuardrailsCount > 0 && (
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={handleSelectAll}>
                  {t("policies:templates.select_all_new", "Select All New")}
                </Button>
                <Button variant="outline" size="sm" onClick={handleDeselectAll}>
                  {t("policies:templates.deselect_all", "Deselect All")}
                </Button>
              </div>
            )}
          </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {guardrail.alreadyExists && <Badge variant="secondary">{t("policies:templates.already_exists", "Already exists")}</Badge>}
                    </div>
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      {guardrail.definition?.litellm_params?.patterns && (
                        <Badge variant="secondary">
                          {t("policies:templates.patterns_count", "{{count}} pattern(s)", { count: guardrail.definition.litellm_params.patterns.length })}
                        </Badge>
                      )}
                      {guardrail.definition?.litellm_params?.categories && (
                        <Badge variant="secondary">
                          {t("policies:templates.categories_count", "{{count}} category/categories", { count: guardrail.definition.litellm_params.categories.length })}
                        </Badge>
                      )}
                    </div>
                  </div>
          ...
          {guardrailsInfo.length === 0 && (
            <div className="py-8 text-center text-muted-foreground">
              <p>{t("policies:templates.no_guardrails_template", "No guardrails defined for this template.")}</p>
              <p className="text-sm mt-2">{t("policies:templates.uses_existing", "This template will use existing guardrails in your system.")}</p>
            </div>
          )}
              ...
              <div className="rounded-lg border border-border bg-muted p-3">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">
                    {t("policies:templates.discovered_competitors", "AI-Discovered Competitors ({{count}})", { count: template.discoveredCompetitors.length })}
                  </span>
                </div>
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {t("policies:templates.discovered_tip", "These competitor names will be automatically blocked by the competitor-name-blocker guardrail.")}
                </p>
              </div>
            </>
          )}
          ...
          <div className="text-sm text-muted-foreground">
            {selectedCount > 0 ? (
              ...
              <p className="text-warning">
                {t("policies:templates.select_at_least_one", "Select at least one guardrail to create, or click \"Use Template\" to proceed without creating new guardrails.")}
              </p>
            )}
          </div>
        </div>
