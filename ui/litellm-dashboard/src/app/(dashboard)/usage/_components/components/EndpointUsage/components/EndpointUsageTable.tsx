...
const EndpointUsageTable: React.FC<EndpointUsageTableProps> = ({ endpointData }) => {
  ...
  const columns: ColumnDef<EndpointRow>[] = [
    {
      header: t("usage:endpoint_usage.table.endpoint", { defaultValue: "Endpoint" }),
      accessorKey: "endpoint",
      cell: ({ row }) => <span className="font-medium">{row.original.endpoint}</span>,
    },
    {
      header: t("usage:endpoint_usage.table.successful_failed", { defaultValue: "Successful / Failed" }),
      id: "requests",
      cell: ({ row }) => {
          ...
          <div className="flex items-center space-x-3">
            <div className="flex-1 relative">
              <Meter
                ...
                aria-label={t("usage:endpoint_usage.table.successful_requests_aria", {
    ...
    {
      header: t("usage:endpoint_usage.table.total_request", { defaultValue: "Total Request" }),
      accessorKey: "api_requests",
      meta: { numeric: true },
      cell: ({ row }) => row.original.api_requests.toLocaleString(),
    },
    {
      header: t("usage:endpoint_usage.table.success_rate", { defaultValue: "Success Rate" }),
    ...
    {
      header: t("usage:endpoint_usage.table.total_tokens", { defaultValue: "Total Tokens" }),
      accessorKey: "total_tokens",
      meta: { numeric: true },
      cell: ({ row }) => row.original.total_tokens.toLocaleString(),
    },
    {
      header: t("usage:endpoint_usage.table.spend", { defaultValue: "Spend" }),
      accessorKey: "spend",
      meta: { numeric: true },
  ...
  return (
    <DataTable
      ...
      noDataMessage={t("usage:endpoint_usage.table.no_data", { defaultValue: "No endpoint usage data" })}
      size="compact"
    />
  );
};
