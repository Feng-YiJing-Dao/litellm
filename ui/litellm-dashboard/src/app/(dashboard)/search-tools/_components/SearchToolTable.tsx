...
function EmptyState({ t }: { t?: (key: any, options?: any) => any }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{tr("tools:search_tools.table.no_tools", { defaultValue: "No search tools configured" })}</div>
      <div className="text-sm text-muted-foreground">{tr("tools:search_tools.table.no_tools_desc", { defaultValue: "Add a search tool to enable web search for your models." })}</div>
    </div>
  );
}
...
const SearchToolTable: React.FC<SearchToolTableProps> = ({
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      loadingMessage={t("tools:search_tools.table.loading", "Loading search tools…")}
