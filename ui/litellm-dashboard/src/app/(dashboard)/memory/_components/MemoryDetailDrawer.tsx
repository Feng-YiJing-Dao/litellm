    ...
    >
      <SheetContent className="overflow-y-auto data-[side=right]:w-full data-[side=right]:max-w-full data-[side=right]:sm:w-[720px] data-[side=right]:sm:max-w-full">
        <SheetHeader className="border-b">
          <SheetTitle>{row ? <code className={CODE_CLASS}>{row.key}</code> : t("drawer.default_title", { defaultValue: "Memory" })}</SheetTitle>
        </SheetHeader>
        {row && (
          <div className="flex flex-col gap-4 px-4 pb-4">
            <div className="flex flex-wrap gap-x-8 gap-y-3">
              <div>
                <span className={`block ${LABEL_CLASS}`}>{t("drawer.memory_id", { defaultValue: "Memory ID" })}</span>
                <code className={CODE_CLASS}>{row.memory_id}</code>
              </div>
              <div>
                <span className={`block ${LABEL_CLASS}`}>{t("drawer.user_id", { defaultValue: "User ID" })}</span>
                <span className={row.user_id ? "text-sm text-foreground" : "text-sm text-muted-foreground"}>
                  {row.user_id ?? "-"}
                </span>
              </div>
              <div>
                <span className={`block ${LABEL_CLASS}`}>{t("drawer.team_id", { defaultValue: "Team ID" })}</span>
            ...
            <div>
              <span className={LABEL_CLASS}>{t("drawer.value", { defaultValue: "Value" })}</span>
              <p className={`${BLOCK_CLASS} text-[13px]`}>{row.value}</p>
            </div>
            {row.metadata !== undefined && row.metadata !== null && (
              <div>
                <span className={LABEL_CLASS}>{t("drawer.metadata", { defaultValue: "Metadata" })}</span>
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              <span>
                {row.created_by
                  ? t("drawer.created_by", {
                      ...
                      user: row.created_by,
                      defaultValue: `Created ${formatTimestamp(row.created_at)} by ${row.created_by}`,
                    })
                  : t("drawer.created", {
                      time: formatTimestamp(row.created_at),
                      defaultValue: `Created ${formatTimestamp(row.created_at)}`,
                    })}
              </span>
              ...
              <span>
                {row.updated_by
                  ? t("drawer.updated_by", {
                      time: formatTimestamp(row.updated_at),
                      user: row.updated_by,
                      defaultValue: `Updated ${formatTimestamp(row.updated_at)} by ${row.updated_by}`,
                    })
                  : t("drawer.updated", {
                      time: formatTimestamp(row.updated_at),
                      defaultValue: `Updated ${formatTimestamp(row.updated_at)}`,
                    })}
              </span>
            </div>
          </div>
