...
const PassThroughGuardrailsSection: React.FC<PassThroughGuardrailsSectionProps> = ({
  ...
  const { t } = useTranslation(["models", "guardrails", "common"]);
    ...
    <TooltipProvider>
      <Card className="block p-6">
        ...
        <p className="mb-6 text-sm text-muted-foreground">
          {t("models:passthrough.guardrails_desc", {
            defaultValue:
        ...
        <Alert variant="info" className="mb-4">
          <Info />
          <AlertTitle>
            {t("models:passthrough.field_level_targeting", { defaultValue: "Field-Level Targeting" })}{" "}
            <a
            ...
            >
              {t("models:passthrough.field_level_learn_more", { defaultValue: "(Learn More)" })}
            </a>
          </AlertTitle>
          <AlertDescription>
            <div className="space-y-2">
              <div>
                {t("models:passthrough.field_level_desc", {
                  defaultValue:
              ...
              <div className="mt-2 space-y-1 text-xs">
                <div className="font-medium">
                  {t("models:passthrough.common_examples", { defaultValue: "Common Examples:" })}
                </div>
                <div>
                  • <code className="rounded-sm bg-muted px-1">query</code> -{" "}
                  {t("models:passthrough.single_field_example", { defaultValue: "Single field" })}
                </div>
                <div>
                  • <code className="rounded-sm bg-muted px-1">documents[*].text</code> -{" "}
                  {t("models:passthrough.docs_field_example", {
                    defaultValue: "All text in documents array",
                  })}
                </div>
                <div>
                  • <code className="rounded-sm bg-muted px-1">messages[*].content</code> -{" "}
                  {t("models:passthrough.messages_field_example", { defaultValue: "All message contents" })}
                </div>
              </div>
            </div>
          </AlertDescription>
        ...
        <Field>
          <FieldLabel htmlFor="pass-through-guardrails">
            {labelWithHint(
              t("models:passthrough.select_guardrails", { defaultValue: "Select Guardrails" }),
              t("models:passthrough.select_guardrails_hint", {
                defaultValue:
          ...
          <div className="mt-6 space-y-4">
            <div className="mb-3 flex items-center justify-between">
              <div className="text-sm font-medium text-foreground">
                {t("models:passthrough.field_targeting_optional", { defaultValue: "Field Targeting (Optional)" })}
              </div>
              <div className="text-xs text-muted-foreground">
                {t("models:passthrough.field_targeting_tip", {
                ...
                <div className="space-y-3">
                  <Field>
                    <div className="flex items-center justify-between">
                      <FieldLabel htmlFor={`${guardrailName}-request-fields`} className="text-xs text-muted-foreground">
                        {labelWithHint(
                          t("models:passthrough.request_fields", { defaultValue: "Request Fields (pre_call)" }),
                          <div>
                            <div className="mb-1 font-medium">
                              {t("models:passthrough.request_fields_hint_title", {
                    ...
                    <TagsInput
                      ...
                      placeholder={t("models:passthrough.request_fields_placeholder", {
                  ...
                  <Field>
                    <div className="flex items-center justify-between">
                      ...
                      >
                        {labelWithHint(
                          t("models:passthrough.response_fields", { defaultValue: "Response Fields (post_call)" }),
                          <div>
                            <div className="mb-1 font-medium">
                              {t("models:passthrough.response_fields_hint_title", {
                    ...
                    <TagsInput
                      ...
                      placeholder={t("models:passthrough.response_fields_placeholder", {
