...
const AdaptiveRoutingConfig: React.FC<AdaptiveRoutingConfigProps> = ({ value, onChange }) => {
  ...
  const qualityCostTitle = t(
    "models:adaptive_routing.quality_cost_title",
    `Quality vs. Cost (${Math.round(adaptiveWeights.quality * 100)}% quality / ${Math.round(adaptiveWeights.cost * 100)}% cost)`,
    {
    ...
    <>
      <Label className="mb-2">
        ...
        <strong className="font-semibold">
          {t("models:adaptive_routing.enable_toggle", "Enable adaptive bandit selection")}
        </strong>
      </Label>
      <span className="block text-xs text-muted-foreground">
        {t(
          "models:adaptive_routing.disabled_hint",
      ...
      <Card className="bg-muted mt-4">
        <CardContent>
          <strong className="mb-2 block font-semibold">
            {t("models:adaptive_routing.how_it_works_title", "How Adaptive Routing Works")}
          </strong>
          <span className="text-[13px] text-muted-foreground">
            {t(
              "models:adaptive_routing.how_it_works_desc",
      ...
      {value.adaptive && (
        <div className="mt-4 space-y-4">
          <div>
            <strong className="mb-1 block font-semibold">{qualityCostTitle}</strong>
            <Slider
              aria-label={qualityCostTitle}
              min={0}
              max={100}
            ...
            <span className="text-xs text-muted-foreground">
              {t(
                "models:adaptive_routing.quality_cost_desc",
          ...
          <div>
            <strong className="mb-1 block font-semibold">
              {t("models:adaptive_routing.eligible_model_pool", "Eligible Model Pool")}
            </strong>
            <RadioGroup
            ...
            >
              <div className="flex w-full flex-col items-start gap-2">
                <Label className="items-start font-normal leading-normal">
                  ...
                  <span>
                    <strong className="font-semibold">
                      {t("models:adaptive_routing.all_tiers_title", "All tiers (soft floor)")}
                    </strong>{" "}
                    <span className="text-muted-foreground">
                      {t(
                        "models:adaptive_routing.all_tiers_desc",
                ...
                <Label className="items-start font-normal leading-normal">
                  ...
                  <span>
                    <strong className="font-semibold">
                      {t("models:adaptive_routing.classified_tier_title", "Classified tier only")}
                    </strong>{" "}
                    <span className="text-muted-foreground">
                      {t(
                        "models:adaptive_routing.classified_tier_desc",
                        "— router can only pick models within tier",
                      )}
                    </span>
                  </span>
                </Label>
          ...
          {adaptiveEligible === "all" && (
            <div>
              <strong className="mb-1 block font-semibold">
                {t("models:adaptive_routing.tier_distance_penalty", "Tier Distance Penalty")}
              </strong>
              <Input
                type="number"
              ...
              <span className="text-xs text-muted-foreground">
                {t(
                  "models:adaptive_routing.tier_distance_penalty_desc",
