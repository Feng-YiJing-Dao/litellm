    ...
    <div className="space-y-4">
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          <p className="text-sm font-medium text-foreground">
            {t("settings:page_visibility.title", { defaultValue: "Internal User Page Visibility" })}
          </p>
          <Badge variant={isPageVisibilitySet ? "secondary" : "outline"}>
            {isPageVisibilitySet
              ? selectedPages.length === 1
                ? t("settings:page_visibility.pages_selected_one", { defaultValue: "1 page selected" })
                : t("settings:page_visibility.pages_selected_other", {
                    count: selectedPages.length,
                    defaultValue: `${selectedPages.length} pages selected`,
                  })
              : t("settings:page_visibility.not_set", { defaultValue: "Not set (all pages visible)" })}
          </Badge>
        </div>
        {enabledPagesPropertyDescription && (
      ...
      <Collapsible className="rounded-lg border border-border">
        <CollapsibleTrigger className="group flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm font-medium hover:bg-muted">
          {t("settings:page_visibility.configure", { defaultValue: "Configure Page Visibility" })}
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="button" onClick={handleSavePageVisibility} disabled={isUpdating}>
                {t("settings:page_visibility.save", { defaultValue: "Save Page Visibility Settings" })}
              </Button>
              {isPageVisibilitySet && (
                <Button type="button" variant="outline" onClick={handleResetToDefault} disabled={isUpdating}>
                  {t("settings:page_visibility.reset", { defaultValue: "Reset to Default (All Pages)" })}
                </Button>
              )}
            </div>
          </div>
