  ...
  return (
    <div className="flex w-full flex-col items-center rounded-lg border border-dashed border-border bg-card p-12 text-center">
      ...
      <h4 className="text-base font-semibold text-foreground">
        {t("settings:sso.empty_title", { defaultValue: "No SSO Configuration Found" })}
      </h4>
      <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
        {t("settings:sso.empty_description", {
          defaultValue:
      ...
      <Button size="lg" onClick={onAdd} className="mt-4">
        {t("settings:sso.configure_button", { defaultValue: "Configure SSO" })}
      </Button>
    </div>
  );
}
