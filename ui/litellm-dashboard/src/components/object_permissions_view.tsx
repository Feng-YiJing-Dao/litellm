...
export function ObjectPermissionsView({
  ...
  const content = (
    <div className={variant === "card" ? "grid grid-cols-1 @xl:grid-cols-2 @4xl:grid-cols-3 gap-6" : "space-y-4"}>
      ...
      <div className="min-w-0 rounded-md border border-border p-4">
        <p className="text-sm font-medium text-foreground">{t("teams:search_tools", { defaultValue: "Search tools" })}</p>
        {searchTools.length === 0 ? (
          <p className="mt-1 block text-xs text-muted-foreground">
            {t("teams:no_restriction_search_tools", { defaultValue: "No restriction — all configured search tools are allowed for this team." })}
          </p>
        ) : (
      ...
      <div className="min-w-0 rounded-md border border-border p-4">
        <p className="text-sm font-medium text-foreground">{t("teams:skills", { defaultValue: "Skills" })}</p>
        {skills.length === 0 ? (
          <p className="mt-1 block text-xs text-muted-foreground">
            {t("teams:no_private_skills_desc", { defaultValue: "No private skills granted. Only enabled (public) Claude Code plugins are visible." })}
          </p>
        ) : (
    ...
    return (
      <div className={`@container bg-card border border-border rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-2 mb-6">
          <div>
            <p className="font-semibold text-foreground">{t("teams:object_permissions", { defaultValue: "Object Permissions" })}</p>
            <p className="text-xs text-muted-foreground">{t("teams:object_permissions_desc", { defaultValue: "Access control for Vector Stores and MCP Servers" })}</p>
          </div>
        </div>
        {content}
      </div>
  ...
  return (
    <div className={`${className}`}>
      <p className="font-medium text-foreground mb-3">{t("teams:object_permissions", { defaultValue: "Object Permissions" })}</p>
      {content}
    </div>
  );
}
