...
const UserDropdown: React.FC<UserDropdownProps> = ({ onLogout, variant = "navbar", collapsed = false }) => {
    ...
    <div className="flex w-full flex-col gap-2 p-3 text-sm">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="left">
                {t("account_menu.upgrade_tooltip", { defaultValue: "Upgrade to Premium for advanced features" })}
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
      ...
      <div className="flex w-full items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted-foreground">{t("account_menu.user_id", { defaultValue: "User ID" })}</span>
        </div>
        <div className="flex items-center gap-1">
          <span className="max-w-[150px] truncate" title={userId || "-"}>
            {userId || "-"}
          </span>
          <CopyButton value={userId} label={t("account_menu.copy_user_id", { defaultValue: "Copy User ID" })} />
        </div>
      </div>
      <div className="flex w-full items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <ShieldCheck className="size-4" />
          <span className="text-muted-foreground">{t("account_menu.role", { defaultValue: "Role" })}</span>
        </div>
        <span>{userRole}</span>
      </div>
      <Separator className="my-2" />
      <div className="flex w-full items-center justify-between gap-2">
        <span className="text-muted-foreground">
          {t("account_menu.hide_new_feature_indicators", { defaultValue: "Hide New Feature Indicators" })}
        </span>
        <Switch
          size="sm"
          ...
          aria-label={t("account_menu.toggle_hide_new_feature_indicators", { defaultValue: "Toggle hide new feature indicators" })}
        />
      </div>
      <div className="flex w-full items-center justify-between gap-2">
        <span className="text-muted-foreground">
          {t("account_menu.hide_all_prompts", { defaultValue: "Hide All Prompts" })}
        </span>
        <Switch
          size="sm"
          ...
          aria-label={t("account_menu.toggle_hide_all_prompts", { defaultValue: "Toggle hide all prompts" })}
        />
      </div>
      <div className="flex w-full items-center justify-between gap-2">
        <span className="text-muted-foreground">
          {t("account_menu.hide_blog_posts", { defaultValue: "Hide Blog Posts" })}
        </span>
        <Switch
          size="sm"
          ...
          aria-label={t("account_menu.toggle_hide_blog_posts", { defaultValue: "Toggle hide blog posts" })}
        />
      </div>
      <div className="flex w-full items-center justify-between gap-2">
        <span className="text-muted-foreground">
          {t("account_menu.hide_bouncing_icon", { defaultValue: "Hide Bouncing Icon" })}
        </span>
        <Switch
          size="sm"
          ...
          aria-label={t("account_menu.toggle_hide_bouncing_icon", { defaultValue: "Toggle hide bouncing icon" })}
        />
      </div>
    </div>
  );
  ...
  const accountAriaLabel = t("account_menu.account_trigger_aria", {
    role: userRole ?? t("unknown", { defaultValue: "Unknown role" }),
    identity: userEmail || userId || t("unknown", { defaultValue: "unknown" }),
    defaultValue: `Account menu — ${userRole ?? "Unknown role"} — signed in as ${userEmail || userId || "unknown"}`,
  });
