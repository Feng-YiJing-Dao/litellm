    ...
    <div className="mt-4">
      <div className="flex items-center mb-2 cursor-pointer" onClick={() => setEntityListExpanded(!entityListExpanded)}>
        ...
        <h4 className="font-medium">
          {t("logs:presidio_entities.detected_entities", `Detected Entities (${entities.length})`, {
                ...
                >
                  <div className="flex items-center">
                    ...
                    <span className={`font-mono ${getScoreColor(entity.score)}`}>
                      {t("logs:presidio_entities.score_prefix", `Score: ${entity.score.toFixed(2)}`, {
                        score: entity.score.toFixed(2),
                      })}
                    </span>
                  </div>
                  <span className="text-xs text-muted-foreground">
                    {t("logs:presidio_entities.position_header", `Position: ${entity.start}-${entity.end}`, {
                      start: entity.start,
                      end: entity.end,
                    })}
                  </span>
                      ...
                      <div className="space-y-2">
                        <div className="flex">
                          <span className="font-medium w-1/3">
                            {t("logs:presidio_entities.entity_type", "Entity Type:")}
                          </span>
                          <span>{entity.entity_type}</span>
                        </div>
                        <div className="flex">
                          <span className="font-medium w-1/3">{t("logs:presidio_entities.position", "Position:")}</span>
                          <span>
                            {t("logs:presidio_entities.characters_range", `Characters ${entity.start}-${entity.end}`, {
                              start: entity.start,
                              end: entity.end,
                            })}
                          </span>
                        ...
                        <div className="flex">
                          <span className="font-medium w-1/3">
                            {t("logs:presidio_entities.confidence", "Confidence:")}
                          </span>
                      ...
                      <div className="space-y-2">
                        {entity.recognition_metadata && (
                          <>
                            <div className="flex">
                              <span className="font-medium w-1/3">
                                {t("logs:presidio_entities.recognizer", "Recognizer:")}
                              </span>
                              <span>{entity.recognition_metadata.recognizer_name}</span>
                            </div>
                            <div className="flex overflow-hidden">
                              <span className="font-medium w-1/3">
                                {t("logs:presidio_entities.identifier", "Identifier:")}
                              </span>
                        ...
                        {entity.analysis_explanation && (
                          <div className="flex">
                            <span className="font-medium w-1/3">
                              {t("logs:presidio_entities.explanation", "Explanation:")}
                            </span>
