...
const ContentFilterDetails: React.FC<ContentFilterDetailsProps> = ({ response }) => {
      ...
      return (
        <div className="bg-card rounded-lg border border-destructive/20 p-4">
          <div className="text-destructive">
            <h5 className="font-medium mb-2">{t("logs:content_filter_details.error", "Error")}</h5>
    ...
    return (
      <div className="bg-card rounded-lg border border-border p-4">
        <div className="text-muted-foreground text-sm">
          {t("logs:content_filter_details.no_detections", "No detections found")}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-card rounded-lg border border-border p-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <KV label={t("logs:content_filter_details.total_detections", "Total Detections:")}>
              <span className="font-semibold">{totalDetections}</span>
            </KV>
            <KV label={t("logs:content_filter_details.actions", "Actions:")}>
              <div className="flex flex-wrap gap-2">
                {blockedCount > 0 &&
                  chip(
                    t("logs:content_filter_details.blocked_count", `${blockedCount} blocked`, { count: blockedCount }),
                    "red",
                  )}
                {maskedCount > 0 &&
                  chip(
                    t("logs:content_filter_details.masked_count", `${maskedCount} masked`, { count: maskedCount }),
                    "blue",
                  )}
                {blockedCount === 0 && maskedCount === 0 && chip(t("logs:content_filter_details.passed", "passed"), "green")}
              </div>
            </KV>
          </div>
          <div className="space-y-2">
            <KV label={t("logs:content_filter_details.by_type", "By Type:")}>
              <div className="flex flex-wrap gap-2">
                {patterns.length > 0 &&
                  chip(
                    t("logs:content_filter_details.patterns_count", `${patterns.length} patterns`, {
                ...
                {blockedWords.length > 0 &&
                  chip(
                    t("logs:content_filter_details.keywords_count", `${blockedWords.length} keywords`, {
                ...
                {categoryKeywords.length > 0 &&
                  chip(
                    t("logs:content_filter_details.categories_count", `${categoryKeywords.length} categories`, {
      ...
      {patterns.length > 0 && (
        <Section
          title={t("logs:content_filter_details.patterns_matched", "Patterns Matched")}
          count={patterns.length}
          defaultOpen={true}
        >
          <div className="space-y-2">
              ...
              <div key={idx} className="p-3 bg-muted rounded-md">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <KV label={t("logs:content_filter_details.pattern_label", "Pattern:")}>
                      {detection.pattern_name || "unknown"}
                    </KV>
                  </div>
                  <div className="space-y-1">
                    <KV label={t("logs:content_filter_details.action_label", "Action:")}>
      ...
      {blockedWords.length > 0 && (
        <Section
          title={t("logs:content_filter_details.blocked_words_detected", "Blocked Words Detected")}
          count={blockedWords.length}
          defaultOpen={true}
        >
          <div className="space-y-2">
            {blockedWords.map((detection, idx) => (
              <div key={idx} className="p-3 bg-muted rounded-md">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <KV label={t("logs:content_filter_details.keyword_label", "Keyword:")} mono>
                      {detection.keyword || "unknown"}
                    </KV>
                    {detection.description && (
                      <KV label={t("logs:content_filter_details.description_label", "Description:")}>
                        {detection.description}
                      </KV>
                    )}
                  </div>
      ...
      {categoryKeywords.length > 0 && (
        <Section
          title={t("logs:content_filter_details.category_keywords_detected", "Category Keywords Detected")}
          count={categoryKeywords.length}
          defaultOpen={true}
        >
          <div className="space-y-2">
            {categoryKeywords.map((detection, idx) => (
              <div key={idx} className="p-3 bg-muted rounded-md">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <KV label={t("logs:content_filter_details.category_label", "Category:")}>
                    ...
                    {detection.severity && (
                      <KV label={t("logs:content_filter_details.severity_label", "Severity:")}>
                        {chip(
                          detection.severity,
      ...
      <Section title={t("logs:content_filter_details.raw_data", "Raw Detection Data")} defaultOpen={false}>
