      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        <CollapsibleTrigger className="flex w-full items-center gap-3 px-4 py-3 text-left">
          ...
          <div className="flex items-center justify-between w-full">
            <h3 className="text-lg font-medium text-foreground">{t("logs:cost_breakdown.title", "Cost Breakdown")}</h3>
            <div className="flex items-center space-x-2 mr-4">
              <span className="text-sm text-muted-foreground">{t("logs:cost_breakdown.total", "Total:")}</span>
              <span className="text-sm font-semibold text-foreground">
                {formatCost(totalSpend)}
                {isCached && ` ${t("logs:cost_breakdown.cached", "(Cached)")}`}
              </span>
            </div>
          </div>
        </CollapsibleTrigger>
        <CollapsibleContent>
          <div className="p-6 space-y-4">
            ...
            <div className="space-y-2 max-w-2xl">
                    ...
                    <>
                      <div className="flex text-sm">
                        <span className="text-muted-foreground font-medium w-1/3">
                          {t("logs:cost_breakdown.input_cost", "Input Cost:")}
                        </span>
                      ...
                      {(costBreakdown?.cache_read_cost ?? 0) > 0 && (
                        <div className="flex text-sm">
                          <span className="text-muted-foreground font-medium w-1/3">
                            {t("logs:cost_breakdown.prompt_cache_read_cost", "Prompt Cache Read Cost:")}
                          </span>
                          <span className="text-foreground">
                      ...
                      {(costBreakdown?.cache_creation_cost ?? 0) > 0 && (
                        <div className="flex text-sm">
                          <span className="text-muted-foreground font-medium w-1/3">
                            {t("logs:cost_breakdown.prompt_cache_write_cost", "Prompt Cache Write Cost:")}
                          </span>
                          <span className="text-foreground">
              ...
              <div className="flex text-sm">
                <span className="text-muted-foreground font-medium w-1/3">
                  {t("logs:cost_breakdown.output_cost", "Output Cost:")}
                </span>
                <span className="text-foreground">
              ...
              {costBreakdown?.tool_usage_cost !== undefined && costBreakdown.tool_usage_cost > 0 && (
                <div className="flex text-sm">
                  <span className="text-muted-foreground font-medium w-1/3">
                    {t("logs:cost_breakdown.tool_usage_cost", "Tool Usage Cost:")}
                  </span>
              ...
              <div className="pt-2 border-t border-border max-w-2xl">
                <div className="flex text-sm font-semibold">
                  <span className="text-foreground w-1/3">
                    {t("logs:cost_breakdown.original_llm_cost", "Original LLM Cost:")}
                  </span>
              ...
              <div className="pt-2 space-y-2 max-w-2xl">
                ...
                {hasDiscount && (
                  <div className="space-y-2">
                    ...
                    {costBreakdown.discount_amount !== undefined && costBreakdown.discount_percent === undefined && (
                      <div className="flex text-sm text-muted-foreground">
                        <span className="font-medium w-1/3">
                          {t("logs:cost_breakdown.discount_amount", "Discount Amount:")}
                        </span>
                        <span className="text-foreground">-{formatCost(costBreakdown.discount_amount)}</span>
                      </div>
                    )}
                ...
                {hasMargin && (
                  <div className="space-y-2">
                    ...
                    {costBreakdown.margin_fixed_amount !== undefined && costBreakdown.margin_fixed_amount !== 0 && (
                      <div className="flex text-sm text-muted-foreground">
                        <span className="font-medium w-1/3">{t("logs:cost_breakdown.margin_fixed", "Margin:")}</span>
            ...
            <div className="mt-4 pt-4 border-t border-border max-w-2xl">
              <div className="flex items-center">
                <span className="font-bold text-sm text-foreground w-1/3">
                  {t("logs:cost_breakdown.final_cost", "Final Calculated Cost:")}
                </span>
