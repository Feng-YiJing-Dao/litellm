...
const MCPServerSelector: React.FC<MCPServerSelectorProps> = ({
  ...
  const effectivePlaceholder = placeholder ?? t("selector.placeholder", { defaultValue: "Select MCP servers" });
  ...
  const options = [
    ...accessGroups.map((group) => ({
      label: group,
      value: group,
      description: t("selector.access_group", { defaultValue: "Access Group" }),
    })),
    ...mcpServers.map((server) => ({
      label: `${server.server_name || server.server_id} (${server.server_id})`,
      value: server.server_id,
      description: t("selector.server", { defaultValue: "MCP Server" }),
    })),
    ...toolsets.map((toolset) => ({
      label: toolset.toolset_name,
      value: `${TOOLSET_PREFIX}${toolset.toolset_id}`,
      description: t("selector.toolset", { defaultValue: "Toolset" }),
    })),
  ];
  ...
  const selectOptions: MultiSelectOption[] = [
    ...(allowAllProxyMcpServers || hasAllProxyMcpServersSelected
      ? [
          {
            label: t("selector.all_proxy", { defaultValue: "All Proxy MCP Servers" }),
    ...
    ...(allowNoMcpServers
      ? [
          {
            label: t("selector.no_servers", { defaultValue: "No MCP Servers" }),
            value: NO_MCP_SERVERS_SENTINEL,
            description: t("selector.block_all", { defaultValue: "Block all" }),
          },
        ]
      : []),
    ...options.map((opt) => ({
  ...
  return (
    <div>
      <MultiSelect
        ...
        emptyText={t("selector.empty", { defaultValue: "No MCP servers found" })}
        loading={loading}
        disabled={disabled}
