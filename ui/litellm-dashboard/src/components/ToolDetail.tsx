...
export function ToolDetail({ toolName, onBack, accessToken }: ToolDetailProps) {
  ...
  const handleInputPolicyChange = useCallback(
    async (_name: string, newPolicy: string) => {
      ...
      try {
        ...
        alert(
          t("policies.detail.failed_update_input", {
            error: e instanceof Error ? e.message : String(e),
            defaultValue: `Failed to update input policy: ${e instanceof Error ? e.message : String(e)}`,
          }),
        );
      } finally {
  ...
  const handleOutputPolicyChange = useCallback(
    async (_name: string, newPolicy: string) => {
      ...
      try {
        ...
        alert(
          t("policies.detail.failed_update_output", {
            error: e instanceof Error ? e.message : String(e),
            defaultValue: `Failed to update output policy: ${e instanceof Error ? e.message : String(e)}`,
          }),
        );
      } finally {
  ...
  const handleAddOverride = useCallback(async () => {
    ...
    try {
      ...
      alert(
        t("policies.detail.failed_add_override", {
          error: e instanceof Error ? e.message : String(e),
          defaultValue: `Failed to add override: ${e instanceof Error ? e.message : String(e)}`,
        }),
      );
    } finally {
  ...
  const handleRemoveOverride = useCallback(
    async (override: ToolPolicyOverrideRow) => {
      ...
      try {
        ...
        alert(
          t("policies.detail.failed_remove_override", {
            error: e instanceof Error ? e.message : String(e),
            defaultValue: `Failed to remove override: ${e instanceof Error ? e.message : String(e)}`,
          }),
        );
      } finally {
        setOverrideSaving(false);
    ...
    return (
      <div>
        <Button variant="link" onClick={onBack} className="mb-4 pl-0">
          <ArrowLeft />
          {t("policies.detail.back", { defaultValue: "Back to Tool Policies" })}
        </Button>
        <p className="text-destructive">
          {t("policies.detail.failed_load", { defaultValue: "Failed to load tool details." })}
        </p>
      </div>
    );
  }
  ...
  return (
    <div>
          ...
          <div>
            <div className="mb-1 flex flex-wrap items-center gap-3">
              ...
              <Badge variant="secondary">
                {t("policies.detail.calls", {
                  count: tool.call_count ?? 0,
                  defaultValue: `${tool.call_count ?? 0} calls`,
                })}
              </Badge>
            </div>
            <dl className="mt-3 flex flex-wrap gap-x-6 gap-y-1 text-sm text-muted-foreground">
              {tool.user_agent && (
                <div className="flex items-center gap-1.5">
                  <dt className="font-medium whitespace-nowrap">
                    {t("policies.detail.user_agent", { defaultValue: "User Agent:" })}
                  </dt>
              ...
              {tool.created_at && (
                <div className="flex items-center gap-1.5">
                  <dt className="font-medium whitespace-nowrap">
                    {t("policies.detail.first_discovered", { defaultValue: "First Discovered:" })}
                  </dt>
              ...
              {tool.last_used_at && (
                <div className="flex items-center gap-1.5">
                  <dt className="font-medium whitespace-nowrap">
                    {t("policies.detail.last_used", { defaultValue: "Last Used:" })}
                  </dt>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <section className="rounded-lg border border-border bg-card p-5 shadow-xs">
            <h2 className="mb-1 text-sm font-semibold">
              {t("policies.detail.input_policy", { defaultValue: "Input Policy" })}
            </h2>
            <p className="mb-3 text-xs text-muted-foreground">
              {inputDesc ??
                t("policies.detail.input_desc_default", {
                  defaultValue: "Controls what data this tool is allowed to accept.",
                })}
            </p>
            <PolicySelect
          ...
          <section className="rounded-lg border border-border bg-card p-5 shadow-xs">
            <h2 className="mb-1 text-sm font-semibold">
              {t("policies.detail.output_policy", { defaultValue: "Output Policy" })}
            </h2>
            <p className="mb-3 text-xs text-muted-foreground">
              {outputDesc ??
                t("policies.detail.output_desc_default", {
                  defaultValue: "Controls how this tool's output is trusted by downstream tools.",
                })}
            </p>
            <PolicySelect
        ...
        {overrides.length > 0 && (
          <section className="rounded-lg border border-border bg-card p-5 shadow-xs">
            <h2 className="mb-3 text-sm font-semibold">
              {t("policies.detail.blocked_for_team_or_key", { defaultValue: "Blocked for team or key" })}
            </h2>
            <ul className="divide-y divide-border rounded-md border border-border">
              {overrides.map((ov) => (
                <li key={ov.override_id} className="flex items-center justify-between px-3 py-2.5 text-sm">
                  <span>
                    {ov.team_id
                      ? t("policies.detail.team_prefix", { id: ov.team_id, defaultValue: `Team: ${ov.team_id}` })
                      : ""}
                    {ov.team_id && ov.key_hash ? " · " : ""}
                    {ov.key_hash
                      ? t("policies.detail.key_prefix", {
                          key: ov.key_alias || ov.key_hash.substring(0, 8),
                          defaultValue: `Key: ${ov.key_alias || ov.key_hash.substring(0, 8)}`,
                        })
                      : ""}
                    {!ov.team_id && !ov.key_hash ? "—" : ""}
                  </span>
                  <Button variant="link" size="sm" disabled={overrideSaving} onClick={() => handleRemoveOverride(ov)}>
                    {t("policies.detail.remove", { defaultValue: "Remove" })}
                  </Button>
                </li>
              ))}
            </ul>
        ...
        <section className="rounded-lg border border-border bg-card p-5 shadow-xs">
          <h2 className="mb-3 text-sm font-semibold">
            {t("policies.detail.block_for_team_or_key", { defaultValue: "Block for team or key" })}
          </h2>
          <div className="flex max-w-md flex-col gap-4">
            <div>
              <span className="mb-2 block text-sm font-medium">
                {t("policies.detail.scope", { defaultValue: "Scope" })}
              </span>
              <div className="flex items-center gap-6">
                <label className="flex cursor-pointer items-center gap-2 text-sm">
                  ...
                  />
                  {t("policies.detail.team", { defaultValue: "Team" })}
                </label>
                <label className="flex cursor-pointer items-center gap-2 text-sm">
                  <input
                    type="radio"
                    ...
                    className="align-middle"
                  />
                  {t("policies.detail.key", { defaultValue: "Key" })}
                </label>
              </div>
            </div>
            <div>
              ...
              {blockScope === "team" ? (
                ...
                >
                  <ComboboxInput
                    placeholder={t("policies.detail.select_key", { defaultValue: "Select key" })}
                    showClear
                    className="w-full min-w-50"
                  />
                  <ComboboxContent>
                    <ComboboxEmpty>
                      {t("policies.detail.no_keys_found", { defaultValue: "No keys found" })}
                    </ComboboxEmpty>
                    <ComboboxList>
                      {(item: KeyItem) => (
            ...
            >
              {t("policies.detail.block_for_scope", {
                scope:
                  blockScope === "team"
                    ? t("policies.detail.team", { defaultValue: "team" })
                    : t("policies.detail.key", { defaultValue: "key" }),
                defaultValue: `Block for ${blockScope}`,
              })}
            </Button>
          </div>
        </section>
        ...
        <section className="rounded-lg border border-border bg-card p-5 shadow-xs">
          <h2 className="mb-3 flex items-center gap-2 text-sm font-semibold">
            ...
            {t("policies.detail.recent_invocations", { defaultValue: "Recent invocations" })}
          </h2>
          <LogViewer
