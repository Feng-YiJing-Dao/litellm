...
const KeySavingsTab: React.FC<KeySavingsTabProps> = ({ accessToken, keyToken, userId, userRole, activity }) => {
  ...
  const intervalLabel = t("key_savings.per_day", { defaultValue: "Per day" });
  const rangeLabel = formatRangeLabel(startTime ?? undefined, endTime ?? undefined);
  const savingsSubtitle = [
    accumulation === "cumulative"
      ? t("key_savings.running_total", { defaultValue: "Running total saved" })
      : t("key_savings.saved_per_interval", {
    ...
    <div className="w-full space-y-6">
      <div className="flex flex-wrap items-center justify-end gap-4">
        <span className="text-sm text-muted-foreground">
          {t("key_savings.utc_day_note", { defaultValue: "Spend is bucketed by UTC day" })}
        </span>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("key_savings.title", { defaultValue: "Savings" })}</CardTitle>
            ...
            <Tabs value={accumulation} onValueChange={(value) => setAccumulation(value as SavingsAccumulation)}>
              <TabsList>
                <TabsTrigger value="cumulative">
                  {t("key_savings.cumulative", { defaultValue: "Cumulative" })}
                </TabsTrigger>
        ...
        <CardContent>
          ...
          {!hasRows && (
            <p className="py-12 text-center text-sm text-muted-foreground" data-testid="key-savings-empty">
              {isLoading
                ? t("key_savings.loading", { defaultValue: "Loading savings..." })
                : t("key_savings.empty", { defaultValue: "No usage recorded for this key in this range." })}
            </p>
          )}
