  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-foreground">{t("permissions.title", { defaultValue: "MCP Servers" })}</p>
        <Badge variant={blocksAllMcpServers ? "destructive" : "secondary"}>
          {blocksAllMcpServers
            ? t("permissions.blocked", { defaultValue: "Blocked" })
            : grantsAllProxyMcpServers
            ? t("permissions.all", { defaultValue: "All" })
            : totalCount}
        </Badge>
      </div>
      ...
      {blocksAllMcpServers ? (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-destructive/10 border border-destructive/20">
          ...
          <p className="text-destructive text-sm">
            {t("permissions.blocked_desc", {
              defaultValue: "No MCP servers — this key is blocked from all MCP servers, including its team's servers",
            })}
          </p>
        </div>
      ) : grantsAllProxyMcpServers ? (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-info/10 border border-info/20">
          <ServerIcon className="h-4 w-4 text-info" />
          <p className="text-info text-sm">{t("permissions.all_proxy_servers", { defaultValue: "All Proxy MCP Servers" })}</p>
        </div>
      ) : totalCount > 0 ? (
                    ...
                    {item.type === "server" ? (
                      ...
                      <div className="inline-flex items-center gap-2 min-w-0">
                        ...
                        <span className="ml-1 px-1.5 py-0.5 text-[9px] font-semibold text-success bg-success/10 border border-success/20 rounded-sm uppercase tracking-wide shrink-0">
                          {t("permissions.group_badge", { defaultValue: "Group" })}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  {hasToolRestrictions && (
                    <div className="flex items-center gap-1 shrink-0 whitespace-nowrap">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {toolsForServer.length === 1
                          ? t("permissions.tool_singular", { defaultValue: "tool" })
                          : t("permissions.tool_plural", { defaultValue: "tools" })}
                      </span>
                      {isExpanded ? (
                  ...
                  >
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      ...
                      <span className="ml-1 px-1.5 py-0.5 text-[9px] font-semibold text-purple-600 bg-purple-50 border border-purple-200 rounded-sm uppercase tracking-wide shrink-0 dark:text-purple-300 dark:bg-purple-950 dark:border-purple-800">
                        {t("permissions.toolset_badge", { defaultValue: "Toolset" })}
                      </span>
                    </div>
                    {toolCount > 0 && (
                      <div className="flex items-center gap-1 shrink-0 whitespace-nowrap">
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted border border-border">
          ...
          <p className="text-muted-foreground text-sm">
            {t("permissions.no_servers_configured", {
              defaultValue: "No MCP servers, access groups, or toolsets configured",
            })}
          </p>
        </div>
      )}
