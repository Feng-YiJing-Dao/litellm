...
const ViewUserSpend: React.FC<ViewUserSpendProps> = ({ userSpend, userMaxBudget, selectedTeam }) => {
  ...
  const displayMaxBudget =
    maxBudget !== null
      ? `$${formatNumberWithCommas(Number(maxBudget), 4)} ${t("usage:limit", { defaultValue: "limit" })}`
      : t("usage:no_limit", { defaultValue: "No limit" });
  ...
  return (
    <div className="flex items-center">
      <div className="flex justify-between gap-x-6">
        <div>
          <p className="text-sm text-muted-foreground">{t("usage:total_spend", { defaultValue: "Total Spend" })}</p>
          <p className="text-2xl font-semibold text-foreground">${roundedSpend}</p>
        </div>
        <div>
          <p className="text-sm text-muted-foreground">{t("usage:max_budget", { defaultValue: "Max Budget" })}</p>
