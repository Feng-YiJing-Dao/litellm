      ...
      <div className="flex flex-col items-end">
        <div className="w-[72%] bg-background border-2 border-primary rounded-xl overflow-hidden shadow-[0_0_0_3px_rgba(var(--primary)/0.1)]">
          ...
          <div className="flex justify-end gap-2 px-2.5 py-1.5 border-t">
            ...
            >
              {t("messages.cancel", { defaultValue: "Cancel" })}
            </Button>
            <Button size="sm" onClick={handleSave} disabled={!editValue.trim()}>
              {t("messages.save_and_send", { defaultValue: "Save & Send" })}
            </Button>
          </div>
        </div>
      </div>
        ...
        {hovered && !isStreaming && onEdit && (
          <TooltipProvider delay={300}>
            <Tooltip>
              ...
              />
              <TooltipContent>
                <p>{t("messages.edit_message", { defaultValue: "Edit message" })}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
...
function AssistantBubble({ message, isLastMessage, isStreaming, isTypingIndicator, mcpEvents }: AssistantBubbleProps) {
  ...
  const stoppedLabel = t("errors.stream_stopped", { defaultValue: "[stopped]" });
  let mainContent = message.content;
  let stoppedSuffix = false;
  if (mainContent.endsWith(stoppedLabel)) {
    mainContent = mainContent.slice(0, -stoppedLabel.length).trimEnd();
    stoppedSuffix = true;
  } else if (mainContent.endsWith("[stopped]")) {
    mainContent = mainContent.slice(0, -"[stopped]".length).trimEnd();
    stoppedSuffix = true;
  } else if (mainContent.endsWith("[已停止]")) {
    mainContent = mainContent.slice(0, -"[已停止]".length).trimEnd();
    stoppedSuffix = true;
  }
    ...
    <div className="flex items-center gap-1 mt-1.5">
      <TooltipProvider delay={300}>
        <Tooltip>
          ...
          />
          <TooltipContent>
            <p>{copied ? t("messages.copied", { defaultValue: "Copied!" }) : t("messages.copy", { defaultValue: "Copy" })}</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    </div>
  ...
  return (
    <>
      ...
      <div className="inline-flex items-center gap-1.5 px-2.5 mb-2 bg-muted/50 border rounded-lg text-xs text-muted-foreground">
        <span className="chat-thinking-text py-1">{t("messages.thinking", { defaultValue: "Thinking..." })}</span>
      </div>
    </>
  );
}
      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        <CollapsibleTrigger className="flex items-center gap-1.5 text-[13px] px-3 py-2 border rounded-lg bg-muted/50 hover:bg-muted transition-colors w-full text-left">
          ...
          <span className="font-medium text-foreground">
            {message.toolName ?? t("messages.tool_call", { defaultValue: "Tool call" })}
          </span>
        </CollapsibleTrigger>
          ...
          {redactedArgs !== undefined && (
            <div className={message.toolResult ? "mb-3" : ""}>
              <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">
                {t("messages.arguments", { defaultValue: "Arguments" })}
              </div>
