    ...
    <InputGroup>
      ...
      <InputGroupAddon align="inline-end">
        <InputGroupButton
          ...
          aria-label={
            revealed
              ? t("logging.hide_password", { defaultValue: "Hide password" })
              : t("logging.show_password", { defaultValue: "Show password" })
          }
        >
          {revealed ? <EyeOff /> : <Eye />}
        </InputGroupButton>
...
const LoggingSettings: React.FC<LoggingSettingsProps> = ({
  ...
  const callbackTypeItems = [
    { value: "success", label: t("logging.callback_success", { defaultValue: "Success Only" }) },
    { value: "failure", label: t("logging.callback_failure", { defaultValue: "Failure Only" }) },
    {
      value: "success_and_failure",
      label: t("logging.callback_success_and_failure", { defaultValue: "Success & Failure" }),
    },
  ];
      ...
      <div className="mt-6 pt-4 border-t border-border">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <span className="text-sm font-medium text-foreground">
            {t("logging.integration_params", { defaultValue: "Integration Parameters" })}
          </span>
        </div>
          ...
          {Object.entries(dynamicParams).map(([paramName, paramType]) => (
            <div key={paramName} className="space-y-2">
              <label className="text-sm font-medium text-foreground capitalize flex items-center space-x-1">
                ...
                {paramType === "password" && (
                  <Badge variant="secondary">{t("logging.badge_sensitive", { defaultValue: "Sensitive" })}</Badge>
                )}
                {paramType === "number" && (
                  <Badge variant="secondary">{t("logging.badge_number", { defaultValue: "Number" })}</Badge>
                )}
              </label>
              {paramType === "number" && (
                <span className="text-xs text-muted-foreground">
                  {t("logging.number_range_hint", { defaultValue: "Value must be between 0 and 1" })}
                </span>
              )}
              {paramType === "number" ? (
                <NumericalInput
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-base font-semibold text-foreground">
            {t("logging.disabled_callbacks_title", { defaultValue: "Disabled Callbacks" })}
          </span>
          <SimpleTooltip
            content={t("logging.disabled_callbacks_tooltip", {
              defaultValue:
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-foreground">
            {t("logging.disabled_callbacks_label", { defaultValue: "Disabled Callbacks" })}
          </label>
          <Select multiple value={disabledCallbacks} onValueChange={handleDisabledCallbacksChange}>
            <SelectTrigger className="w-full">
              <SelectValue
                placeholder={t("logging.disabled_callbacks_placeholder", {
                  defaultValue: "Select callbacks to disable",
                })}
              >
                {(selected: string[]) =>
                  selected.length === 0
                    ? t("logging.disabled_callbacks_placeholder", { defaultValue: "Select callbacks to disable" })
                    : selected.join(", ")
                }
              </SelectValue>
            </SelectTrigger>
          ...
          <div className="text-xs text-muted-foreground">
            {t("logging.disabled_callbacks_desc", {
              defaultValue:
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-base font-semibold text-foreground">
            {t("logging.logging_integrations_title", { defaultValue: "Logging Integrations" })}
          </span>
          <SimpleTooltip
            content={t("logging.logging_integrations_tooltip", {
        ...
        <Button variant="secondary" onClick={addLoggingConfig} size="sm" type="button">
          <Plus />
          {t("logging.add_integration", { defaultValue: "Add Integration" })}
        </Button>
      </div>
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-medium">
                    {callbackDisplayName || t("logging.new_integration", { defaultValue: "New Integration" })}{" "}
                    {t("logging.config_suffix", { defaultValue: "Configuration" })}
                  </span>
                </div>
                <Button
                  variant="ghost"
                ...
                >
                  <Trash2 />
                  {t("logging.remove", { defaultValue: "Remove" })}
                </Button>
              </div>
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground">
                      {t("logging.integration_type", { defaultValue: "Integration Type" })}
                    </label>
                    <Select
                    ...
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue
                          placeholder={t("logging.select_integration", { defaultValue: "Select integration" })}
                        />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground">
                      {t("logging.event_type", { defaultValue: "Event Type" })}
                    </label>
                    <Select
                    ...
                    >
                      <SelectTrigger
                        aria-label={t("logging.event_type", { defaultValue: "Event Type" })}
                        className="w-full"
                      >
                        <SelectValue />
                      </SelectTrigger>
      ...
      {value.length === 0 && (
        <div className="text-center py-12 text-muted-foreground border-2 border-dashed border-border rounded-lg bg-muted/30">
          ...
          <div className="text-base font-medium mb-1">
            {t("logging.no_integrations_title", { defaultValue: "No logging integrations configured" })}
          </div>
          <div className="text-sm text-muted-foreground">
            {t("logging.no_integrations_desc", {
