...
const ModelGroupAliasSettings: React.FC<ModelGroupAliasSettingsProps> = ({
  ...
  const saveAliasesToBackend = async (updatedAliases: AliasItem[]) => {
    ...
    try {
      ...
      toast.fromError(
        t("models:group_alias.err_save", { defaultValue: "Failed to save model group alias settings" }),
      );
      return false;
    }
  };
  ...
  const handleAddAlias = async () => {
    if (!newAlias.aliasName || !newAlias.targetModelGroup) {
      toast.fromError(
        t("models:group_alias.err_missing_fields", {
          defaultValue: "Please provide both alias name and target model group",
        }),
      );
      return;
    }
    ...
    if (aliases.some((alias) => alias.aliasName === newAlias.aliasName)) {
      toast.fromError(
        t("models:group_alias.err_duplicate", { defaultValue: "An alias with this name already exists" }),
      );
      return;
    }
    ...
    if (await saveAliasesToBackend(updatedAliases)) {
      ...
      toast.success(t("models:group_alias.success_add", { defaultValue: "Alias added successfully" }));
    }
  };
  ...
  const handleUpdateAlias = async () => {
    ...
    if (await saveAliasesToBackend(updatedAliases)) {
      ...
      toast.success(t("models:group_alias.success_update", { defaultValue: "Alias updated successfully" }));
    }
  };
  ...
  const deleteAlias = async (aliasId: string) => {
    ...
    if (await saveAliasesToBackend(updatedAliases)) {
      setAliases(updatedAliases);
      toast.success(t("models:group_alias.success_delete", { defaultValue: "Alias deleted successfully" }));
    }
  };
      ...
      <div className="flex items-center justify-between cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="flex flex-col">
          <CardTitle className="mb-0">
            {t("models:group_alias.title", { defaultValue: "Model Group Alias Settings" })}
          </CardTitle>
          <p className="text-sm text-muted-foreground">
            {t("models:group_alias.desc", {
              defaultValue:
        ...
        <div className="mt-4">
          <div className="mb-6">
            <p className="text-sm font-medium text-foreground mb-2">
              {t("models:group_alias.add_new", { defaultValue: "Add New Alias" })}
            </p>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="block text-xs text-muted-foreground mb-1">
                  {t("models:group_alias.alias_name", { defaultValue: "Alias Name" })}
                </label>
                <input
                  type="text"
                  ...
                  placeholder={t("models:group_alias.alias_placeholder", { defaultValue: "e.g., gpt-4o" })}
                  className="w-full px-3 py-2 border border-border rounded-md text-sm"
                />
              </div>
              <div>
                <label className="block text-xs text-muted-foreground mb-1">
                  {t("models:group_alias.target_group", { defaultValue: "Target Model Group" })}
                </label>
                <input
                  type="text"
                  ...
                  placeholder={t("models:group_alias.target_placeholder", {
                    defaultValue: "e.g., gpt-4o-mini-openai",
                  })}
                  className="w-full px-3 py-2 border border-border rounded-md text-sm"
                />
              </div>
              <div className="flex items-end">
                ...
                >
                  ...
                  {t("models:group_alias.add_btn", { defaultValue: "Add Alias" })}
                </button>
              </div>
            </div>
          </div>
          ...
          <p className="text-sm font-medium text-foreground mb-2">
            {t("models:group_alias.manage_title", { defaultValue: "Manage Existing Aliases" })}
          </p>
              ...
              <Table className="[&_td]:py-0.5 [&_th]:py-1">
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="py-1 h-8">
                      {t("models:group_alias.actions", { defaultValue: "Actions" })}
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  ...
                  {aliases.length === 0 && (
                    <TableRow>
                      ...
                      >
                        {t("models:group_alias.no_aliases", {
          ...
          <Card className="px-6">
            <CardTitle className="mb-4">
              {t("models:group_alias.config_example_title", { defaultValue: "Configuration Example" })}
            </CardTitle>
            <p className="text-muted-foreground mb-4">
              {t("models:group_alias.config_example_desc", {
                defaultValue: "Here's how your current aliases would look in the config.yaml:",
              })}
            </p>
              ...
              <div className="text-foreground">
                ...
                {Object.keys(aliasObject).length === 0 ? (
                  <span className="text-muted-foreground">
                    <br />
                    &nbsp;&nbsp;&nbsp;&nbsp;
                    {t("models:group_alias.no_aliases_comment", { defaultValue: "# No aliases configured yet" })}
                  </span>
                ) : (
